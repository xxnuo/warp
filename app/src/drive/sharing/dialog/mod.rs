...
impl SharingDialog {
    fn access_level_label(access_level: SharingAccessLevel) -> String {
        match access_level {
            SharingAccessLevel::View => tr("sharing.access.view"),
            SharingAccessLevel::Edit => tr("sharing.access.edit"),
            SharingAccessLevel::Full => tr("sharing.access.full"),
        }
    }
    ...
    pub fn new(target: Option<ShareableObject>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let invite_form = EmailInviteForm {
            email_editor: ctx.add_typed_action_view(|ctx| {
                let emails_placeholder = tr("sharing.emails");
    ...
    fn render_invite_form(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let mut invite_button = appearance
            ...
            .with_centered_text_label(tr("sharing.invite"))
        ...
        let contents = if validation_state.is_valid() {
            ...
            if !validation_state.duplicate_guests.is_empty() {
                let emails = validation_state
                    .duplicate_guests
                    .iter()
                    .format(", ")
                    .to_string();
                let error_text = tr_with("sharing.already_shared_with", &[("emails", &emails)]);
            ...
            if !validation_state.invalid_emails.is_empty() {
                let emails = validation_state
                    ...
                    .format(", ")
                    .to_string();
                let error_text = tr_with("sharing.invalid_address", &[("emails", &emails)]);
                contents.add_child(self.render_error_message(error_text, appearance));
            }
    ...
    fn render_access_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        appearance
            .ui_builder()
            .span(tr("sharing.who_has_access"))
            .with_style(UiComponentStyles {
    ...
    fn render_restricted_access_label(
        ...
        let prefix = tr("sharing.full_access_required_prefix");
        let suffix = tr("sharing.full_access_required_suffix");
        let access_level_label = Self::access_level_label(access_level);
        let message = format!("{prefix}{access_level_label}{suffix}");
        let access_level_start = prefix.chars().count();
        let access_level_end = access_level_start + access_level_label.chars().count();
    ...
    fn render_owner(&self, appearance: &Appearance, app: &AppContext) -> Option<Box<dyn Element>> {
        ...
        let tooltip_text = match owner {
            Subject::Team(_) => tr("sharing.owner_tooltip.team"),
            _ => tr("sharing.owner_tooltip.default"),
        };
    ...
    fn reset_link_sharing_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let mut items = vec![
            MenuItemFields::new(tr("sharing.only_people_invited"))
                .with_on_select_action(SharingDialogAction::SetLinkPermissions(None))
                .with_icon(Icon::Lock)
                .with_disabled(inherited_access)
                .into_item(),
            MenuItem::Separator,
            MenuItemFields::new(tr("sharing.anyone_with_link"))
    ...
    fn reset_team_sharing_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let items = [
            MenuItemFields::new(tr("sharing.only_invited_teammates"))
                .with_on_select_action(SharingDialogAction::SetTeamPermissions(None))
                .with_icon(Icon::Lock)
                .with_disabled(inherited_access)
                .into_item(),
            MenuItem::Separator,
            MenuItemFields::new(tr("sharing.teammates_with_link"))
                .with_no_interaction_on_hover()
    ...
    fn render_subject(
        ...
        let name_text = subject
            .name(app)
            .unwrap_or_else(|| Cow::Owned(tr("common.unknown")));
    ...
    fn download_qr_code(&self, ctx: &mut ViewContext<Self>) {
        let Some(url) = self.target_link(ctx) else {
            self.show_ephemeral_toast(
                DismissibleToast::error(tr("sharing.unable_download_qr_code")),
                ctx,
            );
            return;
        };
    ...
    fn handle_qr_write_result(&self, result: std::io::Result<()>, ctx: &mut ViewContext<Self>) {
        match result {
            Ok(()) => self.show_ephemeral_toast(
                DismissibleToast::success(tr("sharing.qr_code_downloaded")),
                ctx,
            ),
    ...
    fn render_qr_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let title = appearance
            .ui_builder()
            .span(tr("sharing.share_session_qr_code"))
            .with_style(UiComponentStyles {
                font_color: Some(foreground),
                font_size: Some(style::HEADER_TEXT_SIZE),
                ..Default::default()
    ...
    fn render_qr_dialog(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let qr_contents = self
            ...
            .unwrap_or_else(|| {
                appearance
                    .ui_builder()
                    .paragraph(tr("sharing.unable_create_qr_code"))
                    .with_style(UiComponentStyles {
                        font_color: Some(style::acl_secondary_text_color(appearance)),
                        ..Default::default()
                    })
