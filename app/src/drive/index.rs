...
fn cloud_object_type_label(object_type: ObjectType) -> String {
    match object_type {
        ObjectType::Workflow => tr("drive.object_type.workflows"),
        ObjectType::Notebook => tr("drive.object_type.notebooks"),
        ObjectType::Folder => tr("drive.object_type.folders"),
        ObjectType::GenericStringObject(_) => tr("drive.object_type.objects"),
    }
}
...
impl DriveIndex {
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let title_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(&tr("drive.untitled"), ctx);
            editor
        });
    ...
    fn render_trash_section_header(
        ...
        let mut empty_trash_button = appearance
            ...
            .with_text_label(tr("drive.empty_trash"));
            ...
            (DriveIndexVariant::MainIndex, DriveIndexSection::CreateATeam) => {
                if self.is_online(app) {
                    Some(self.render_team_section_header(
                        tr("drive.create_team.description"),
                        appearance,
                    ))
                } else {
                    None
            ...
            (DriveIndexVariant::MainIndex, DriveIndexSection::JoinTeam) => {
                if self.is_online(app) {
                    let teammate_count = UserWorkspaces::handle(app)
                        ...
                        .to_string();
                    let join_teams_text =
                        tr_with("drive.join_team.description", &[("count", &teammate_count)]);
    ...
    fn render_trash_row(&self, appearance: &Appearance, _: &AppContext) -> Box<dyn Element> {
        ...
        let title = Container::new(
            appearance
                .ui_builder()
                .wrappable_text(tr("drive.trash_upper"), false)
                .with_style(UiComponentStyles {
                    font_family_id: Some(appearance.ui_font_family()),
                    font_size: Some(SECTION_HEADER_FONT_SIZE),
                    font_weight: Some(Weight::Normal),
    ...
    fn render_team_space_zero_state(&self, appearance: &Appearance) -> Box<dyn Element> {
        let zero_state_info = Container::new(
            appearance
                .ui_builder()
                .wrappable_text(tr("drive.team_zero_state.hint"), true)
                .with_style(UiComponentStyles {
                    font_family_id: Some(appearance.ui_font_family()),
                    font_size: Some(ITEM_FONT_SIZE),
    ...
    fn render_create_team_section(
        ...
        let button_text = tr("settings.teams.create_team");
    ...
    fn render_join_discoverable_team_section(
        ...
        let text = if UserWorkspaces::as_ref(app).num_joinable_teams() > 1 {
            tr("drive.join_team.view_teams")
        } else {
            tr("drive.join_team.view_team")
        };
        ...
        let or_text = Container::new(
            Text::new_inline(tr("common.or"), appearance.ui_font_family(), ITEM_FONT_SIZE)
    ...
    fn render_title(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let text = Container::new(
            appearance
                .ui_builder()
                .span(tr("settings.section.warp_drive"))
                .with_style(UiComponentStyles {
                    font_family_id: Some(appearance.ui_font_family()),
            ...
            |mouse_state| {
                ...
                if mouse_state.is_hovered() {
                    let tooltip = appearance
                        .ui_builder()
                        .tool_tip(tr("drive.syncing_warp_drive"));
    ...
    fn render_retry_button(&self, appearance: &Appearance) -> Box<dyn warpui::Element> {
        ...
        .with_tooltip(move || ui_builder.tool_tip(tr("drive.retry_sync")).build().finish())
        .build()
        .on_click(move |ctx, _, _| {
    ...
    fn toggle_new_assets_menu(&mut self, space: &Space, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(state) = state {
            ...
            if is_online {
                menu_items.push(
                    MenuItemFields::new(tr("drive.object_type.folder"))
            ...
            menu_items.push(
                MenuItemFields::new(tr("drive.object_type.workflow"))
                    .with_on_select_action(DriveIndexAction::create_object(
                        DriveObjectType::Workflow,
                        *space,
                        None,
            ...
            if FeatureFlag::AgentModeWorkflows.is_enabled() {
                menu_items.push(
                    MenuItemFields::new(tr("drive.object_type.prompt"))
                        .with_on_select_action(DriveIndexAction::create_object(
                            DriveObjectType::AgentModeWorkflow,
                            *space,
                            None,
            ...
            menu_items.push(
                MenuItemFields::new(tr("drive.object_type.notebook"))
                    .with_on_select_action(DriveIndexAction::create_object(
                        DriveObjectType::Notebook {
                            is_ai_document: false,
                        },
            ...
            menu_items.push(
                MenuItemFields::new(tr("drive.object_type.env_var_collection"))
                    .with_on_select_action(DriveIndexAction::create_object(
                        DriveObjectType::EnvVarCollection,
                        *space,
                        None,
            ...
            menu_items.push(
                MenuItemFields::new(tr("drive.import.title"))
    ...
    fn render_personal_limit_status(
        ...
        let header = Flex::row()
            ...
            .with_child(
                Text::new_inline(
                    tr("settings.section.warp_drive"),
                    appearance.ui_font_family(),
                    14.,
                )
        ...
        let personal_object_limit_description = tr("drive.personal_limit_description");
    ...
    fn render_personal_object_limit_row(
        ...
        let name = match object_type {
            DriveObjectType::Notebook { .. } => tr("drive.object_type.notebooks"),
            DriveObjectType::Workflow => tr("drive.object_type.workflows"),
            DriveObjectType::EnvVarCollection => tr("drive.object_type.env_var_collections"),
            DriveObjectType::Folder => tr("drive.object_type.folders"),
            DriveObjectType::AgentModeWorkflow => tr("drive.object_type.agent_workflows"),
            DriveObjectType::AIFact => tr("drive.object_type.ai_fact"),
            DriveObjectType::AIFactCollection => tr("drive.object_type.rules"),
            DriveObjectType::MCPServer => tr("drive.object_type.mcp_server"),
            DriveObjectType::MCPServerCollection => tr("settings.section.mcp_servers"),
        };
    ...
    fn render_shared_object_limit_hit_banner(
        ...
        let object_type = cloud_object_type_label(object_type);
        let banner_line_1 = tr_with(
            "drive.shared_object_limit.line_1",
            &[("object_type", &object_type)],
        );
        let banner_line_2 = tr("drive.shared_object_limit.line_2");
        let banner_text = format!("{banner_line_1} {banner_line_2}");
        ...
        let button = appearance
            ...
            .with_centered_text_label(tr("settings.account.compare_plans"))
            .with_style(UiComponentStyles {
                font_size: Some(14.),
                font_weight: Some(Weight::Light),
                padding: Some(Coords {
    ...
    fn render_payment_issue_banner(
        ...
        let banner_line_1 = tr("drive.payment_issue.line_1");
        let banner_line_2 = if has_admin_permissions && is_on_stripe_paid_plan {
            tr("drive.payment_issue.line_2_admin")
        } else if has_admin_permissions && !is_on_stripe_paid_plan {
            tr("drive.payment_issue.line_2_admin_enterprise")
        } else {
            tr("drive.payment_issue.line_2_nonadmin")
        };
        let banner_text = format!("{banner_line_1} {banner_line_2}");
        ...
        if has_admin_permissions && is_on_stripe_paid_plan {
            let button = appearance
                ...
                .with_centered_text_label(tr("settings.account.manage_billing"))
                .with_style(UiComponentStyles {
                    font_size: Some(14.),
                    font_weight: Some(Weight::Light),
                    padding: Some(Coords {
        ...
        if let CloudObjectTypeAndId::Folder(folder_id) = cloud_object_type_and_id {
            if let SyncId::ServerId(_) = folder_id {
                if self.is_online(app) {
                    if !FeatureFlag::SharedWithMe.is_enabled() || editability.can_edit() {
                        menu_items.push(
                            MenuItemFields::new(tr("drive.new.folder"))
                                .with_on_select_action(DriveIndexAction::create_object(
                                    DriveObjectType::Folder,
                                    *space,
                                    Some(*folder_id),
                        ...
                        menu_items.push(
                            MenuItemFields::new(tr("drive.new.workflow"))
                                .with_on_select_action(DriveIndexAction::create_object(
                                    DriveObjectType::Workflow,
                                    *space,
                                    Some(*folder_id),
                        ...
                        if FeatureFlag::AgentModeWorkflows.is_enabled() {
                            menu_items.push(
                                MenuItemFields::new(tr("drive.new.prompt"))
                                    .with_on_select_action(DriveIndexAction::create_object(
                                        DriveObjectType::AgentModeWorkflow,
                                        *space,
                                        Some(*folder_id),
                        ...
                        menu_items.push(
                            MenuItemFields::new(tr("drive.new.notebook"))
                                .with_on_select_action(DriveIndexAction::create_object(
                                    DriveObjectType::Notebook {
                                        is_ai_document: false,
                                    },
                        ...
                        menu_items.push(
                            MenuItemFields::new(tr("drive.new.env_var_collection"))
                                .with_on_select_action(DriveIndexAction::create_object(
                                    DriveObjectType::EnvVarCollection,
                                    *space,
                                    Some(*folder_id),
                ...
                if let Some(object) = object {
                    if let Some(object_link) = object.object_link() {
                        menu_items.push(
                            MenuItemFields::new(tr("sharing.copy_link"))
                        ...
                        if editability.can_edit() {
                            menu_items.push(
                                MenuItemFields::new(tr("sharing.share"))
                ...
                menu_items.push(
                    MenuItemFields::new(tr("drive.menu.collapse_all"))
            ...
            if let Some(object) = object {
                if self.is_online(app) && object.metadata().is_errored() {
                    menu_items.push(
                        MenuItemFields::new(tr("common.retry"))
                    ...
                    if let Some(server_id) = cloud_object_type_and_id.server_id() {
                        menu_items.push(
                            MenuItemFields::new(tr("drive.menu.revert_to_server"))
                ...
                match object.object_type() {
                    ObjectType::Workflow => {
                        ...
                        let label = if workflow.model().data.is_agent_mode_workflow() {
                            tr("drive.menu.copy_prompt")
                        } else {
                            tr("drive.menu.copy_workflow_text")
                        };
                        menu_items.push(
                        ...
                        if workflow.model().data.is_agent_mode_workflow() {
                            menu_items.push(
                                MenuItemFields::new(tr("drive.menu.copy_id"))
                    ...
                    ObjectType::GenericStringObject(GenericStringObjectFormat::Json(
                        ...
                        menu_items.push(
                            MenuItemFields::new(tr("drive.menu.copy_variables"))
                                .with_on_select_action(DriveIndexAction::CopyObjectToClipboard(
                                    *cloud_object_type_and_id,
                                ))
                                .with_icon(Icon::CopyMenuItem)
                        ...
                        menu_items.push(
                            MenuItemFields::new(tr("drive.menu.load_in_subshell"))
                                .with_on_select_action(
                                ...
                                == UserAppInstallStatus::Detected
                        {
                            if let Some(object_link) = object.object_link() {
                                if let Ok(url) = Url::parse(&object_link) {
                                    menu_items.push(
                                        MenuItemFields::new(tr("drive.menu.open_on_desktop"))
                                            .with_on_select_action(
    ...
    fn pane_menu_item(
        ...
        if (FeatureFlag::SharedWithMe.is_enabled() && !editability.can_edit()) || prefer_open {
            MenuItemFields::new(tr("common.open")).with_icon(Icon::Eye)
        } else {
            MenuItemFields::new(tr("common.edit")).with_icon(Icon::Rename)
        }
    }
    ...
    fn trash_menu_items(
        ...
        if self.online_only_operation_allowed(cloud_object_type_and_id, app) {
            if !FeatureFlag::SharedWithMe.is_enabled() || access_level.can_trash() {
                menu_items.push(
                    MenuItemFields::new(tr("common.restore"))
            ...
            if !FeatureFlag::SharedWithMe.is_enabled() || access_level.can_delete() {
                menu_items.push(
                    MenuItemFields::new(tr("drive.menu.delete_forever"))
    ...
    fn toggle_space_menu(&mut self, space: &Space, offset: Vector2F, ctx: &mut ViewContext<Self>) {
        ...
        let menu_items = vec![MenuItemFields::new(tr("drive.menu.collapse_all"))
            .with_on_select_action(DriveIndexAction::CollapseAllInLocation(
                CloudObjectLocation::Space(*space),
            ))
            .with_icon(Icon::ListCollapsed)
