...
impl CloudObjectNamingDialog {
    ...
    fn render_text_header(
        ...
        let title = match object_type {
            DriveObjectType::Notebook { .. } => tr("drive.naming.notebook_name"),
            DriveObjectType::Folder => tr("drive.naming.folder_name"),
            DriveObjectType::EnvVarCollection => tr("drive.naming.collection_name"),
    ...
    fn render_action_buttons(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let primary_button_text = match self.is_rename {
            true => tr("common.rename"),
            false => tr("common.create"),
        };
