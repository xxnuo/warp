...
const ENABLE_SHELL_DEBUG_MODE_MENU_ITEM_NAME: &str = "app_menu.enable_shell_debug_mode";
const DISABLE_SHELL_DEBUG_MODE_MENU_ITEM_NAME: &str = "app_menu.disable_shell_debug_mode";
const ENABLE_IN_BAND_GENERATORS_MENU_ITEM_NAME: &str = "app_menu.enable_in_band_generators";
const DISABLE_IN_BAND_GENERATORS_MENU_ITEM_NAME: &str = "app_menu.disable_in_band_generators";
const ENABLE_PTY_RECORDING: &str = "app_menu.enable_pty_recording";
const DISABLE_PTY_RECORDING: &str = "app_menu.disable_pty_recording";
const SHOW_BOOTSTRAP_BLOCK_MENU_ITEM_NAME: &str = "app_menu.show_initialization_block";
const HIDE_BOOTSTRAP_BLOCK_MENU_ITEM_NAME: &str = "app_menu.hide_initialization_block";
const SHOW_IN_BAND_COMMAND_BLOCKS_MENU_ITEM_NAME: &str = "app_menu.show_in_band_command_blocks";
const HIDE_IN_BAND_COMMAND_BLOCKS_MENU_ITEM_NAME: &str = "app_menu.hide_in_band_command_blocks";
const SHOW_SSH_COMMAND_BLOCKS_MENU_ITEM_NAME: &str = "app_menu.show_warpified_ssh_blocks";
const HIDE_SSH_COMMAND_BLOCKS_MENU_ITEM_NAME: &str = "app_menu.hide_warpified_ssh_blocks";
const EXPORT_DEFAULT_SETTINGS_CSV_MENU_ITEM_NAME: &str = "app_menu.export_default_settings_csv";
...
pub fn dock_menu() -> Menu {
    let new_window = tr("app_menu.new_window");
    Menu::new(
        new_window.clone(),
...
fn make_new_app_menu(ctx: &AppContext) -> Menu {
    ...
    menu_items.push(MenuItem::Custom(CustomMenuItem::new(
        &tr("app_menu.set_warp_default_terminal"),
        move |ctx| {
...
fn make_new_file_menu(ctx: &AppContext) -> Menu {
    ...
    Menu::new(tr("app_menu.file"), file_menu_options)
}
...
fn make_new_edit_menu(ctx: &AppContext) -> Menu {
    ...
    let group_5 = vec![
        ...
        MenuItem::Custom(CustomMenuItem::new(
            &tr("app_menu.copy_on_select_terminal"),
            move |ctx| {
    ...
    edit_menu_items.push(MenuItem::Custom(CustomMenuItem::new_with_submenu(
        &tr("app_menu.synchronize_inputs"),
        |_| (),
        no_updates,
        None,
        vec![
    ...
    Menu::new(tr("app_menu.edit"), edit_menu_items)
}
...
fn make_new_view_menu(ctx: &AppContext) -> Menu {
    let mut items = vec![
        ...
        MenuItem::Separator,
        MenuItem::Custom(CustomMenuItem::new(
            &tr("app_menu.toggle_mouse_reporting"),
            move |ctx| {
        ...
        MenuItem::Custom(CustomMenuItem::new(
            &tr("app_menu.toggle_scroll_reporting"),
            move |ctx| {
        ...
        MenuItem::Custom(CustomMenuItem::new(
            &tr("app_menu.toggle_focus_reporting"),
            move |ctx| {
    ...
    Menu::new(tr("app_menu.view"), items)
}
...
fn make_new_tab_menu(ctx: &AppContext) -> Menu {
    ...
    Menu::new(tr("app_menu.tab"), items)
}
...
fn make_new_blocks_menu(ctx: &AppContext) -> Menu {
    ...
    Menu::new(tr("app_menu.blocks"), items)
}
...
fn make_new_drive_menu(ctx: &AppContext) -> Menu {
    ...
    Menu::new(tr("app_menu.drive"), items)
}
...
fn block_menu_debug_items() -> Vec<MenuItem> {
    ...
    items.push(MenuItem::Custom(CustomMenuItem::new(
        &tr(SHOW_IN_BAND_COMMAND_BLOCKS_MENU_ITEM_NAME),
        move |ctx| {
        ...
        move |_props, ctx| {
            let name = if BlockVisibilitySettings::handle(ctx).read(ctx, |settings, _ctx| {
                *settings.should_show_in_band_command_blocks.value()
                ...
                tr(HIDE_IN_BAND_COMMAND_BLOCKS_MENU_ITEM_NAME)
            } else {
                tr(SHOW_IN_BAND_COMMAND_BLOCKS_MENU_ITEM_NAME)
            };
    ...
    items.push(MenuItem::Custom(CustomMenuItem::new(
        &tr(SHOW_SSH_COMMAND_BLOCKS_MENU_ITEM_NAME),
        move |ctx| {
            let handle = BlockVisibilitySettings::handle(ctx);
            handle.update(ctx, |block_visibility_settings, ctx| {
        ...
        move |_props, ctx| {
            let name = if BlockVisibilitySettings::handle(ctx).read(ctx, |settings, _ctx| {
                *settings.should_show_ssh_block.value()
                ...
                tr(HIDE_SSH_COMMAND_BLOCKS_MENU_ITEM_NAME)
            } else {
                tr(SHOW_SSH_COMMAND_BLOCKS_MENU_ITEM_NAME)
            };
...
fn toggle_bootstrap_block_menu_item() -> MenuItem {
    MenuItem::Custom(CustomMenuItem::new(
        &tr(SHOW_BOOTSTRAP_BLOCK_MENU_ITEM_NAME),
        move |ctx| {
        ...
        move |_props, ctx| {
            let name = if BlockVisibilitySettings::handle(ctx).read(ctx, |settings, _ctx| {
                *settings.should_show_bootstrap_block.value()
                ...
                tr(HIDE_BOOTSTRAP_BLOCK_MENU_ITEM_NAME)
            } else {
                tr(SHOW_BOOTSTRAP_BLOCK_MENU_ITEM_NAME)
            };
...
fn debug_menu_items() -> Vec<MenuItem> {
    ...
    if FeatureFlag::DebugMode.is_enabled() {
        debug_menu_items.push(MenuItem::Custom(CustomMenuItem::new(
            &tr(ENABLE_SHELL_DEBUG_MODE_MENU_ITEM_NAME),
            move |ctx| {
            ...
            move |_props, ctx| {
                let name = if DebugSettings::handle(ctx).read(ctx, |settings, _ctx| {
                    *settings.is_shell_debug_mode_enabled.value()
                    ...
                    tr(DISABLE_SHELL_DEBUG_MODE_MENU_ITEM_NAME)
                } else {
                    tr(ENABLE_SHELL_DEBUG_MODE_MENU_ITEM_NAME)
                };
        ...
        debug_menu_items.push(MenuItem::Custom(CustomMenuItem::new(
            &tr(ENABLE_IN_BAND_GENERATORS_MENU_ITEM_NAME),
            move |ctx| {
                DebugSettings::handle(ctx).update(ctx, |debug_settings, ctx| {
            ...
            move |_props, ctx| {
                let name = if DebugSettings::handle(ctx).read(ctx, |settings, _ctx| {
                    *settings
                    ...
                    tr(DISABLE_IN_BAND_GENERATORS_MENU_ITEM_NAME)
                } else {
                    tr(ENABLE_IN_BAND_GENERATORS_MENU_ITEM_NAME)
                };
        ...
        debug_menu_items.push(MenuItem::Custom(CustomMenuItem::new(
            &tr("app_menu.manually_toggle_network_status"),
        ...
        debug_menu_items.push(MenuItem::Custom(CustomMenuItem::new(
            &tr(EXPORT_DEFAULT_SETTINGS_CSV_MENU_ITEM_NAME),
            move |ctx| {
        ...
        debug_menu_items.push(MenuItem::Custom(CustomMenuItem::new(
            &tr("app_menu.create_anonymous_user"),
...
fn make_new_help_menu() -> Menu {
    Menu::new(
        ...
        vec![
            feedback_menu_item(),
            link_menu_item(
                tr("app_menu.warp_documentation"),
                links::USER_DOCS_URL.into(),
            ),
            link_menu_item(
                tr("app_menu.github_issues"),
                links::GITHUB_ISSUES_URL.into(),
            ),
            link_menu_item(tr("app_menu.warp_slack_community"), links::SLACK_URL.into()),
        ],
    )
}
...
fn make_new_elements_menu_items(ctx: &AppContext) -> Vec<MenuItem> {
    ...
    let mut new_elements_menu = vec![
        MenuItem::Custom(CustomMenuItem::new(
            &tr("app_menu.new_window"),
            open_new_window,
            no_updates,
            Some(Keystroke::parse("cmd-n").expect("Valid keystroke")),
        )),
        MenuItem::Custom(CustomMenuItem::new(
            &tr("app_menu.new_terminal_tab"),
    ...
    new_elements_menu.push(MenuItem::Custom(CustomMenuItem::new(
        &tr("app_menu.reopen_closed_session"),
        |ctx| {
    ...
    new_elements_menu.push(MenuItem::Custom(CustomMenuItem::new_with_submenu(
        &tr("app_menu.launch_configurations"),
        |_| (),
