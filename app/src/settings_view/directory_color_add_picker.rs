...
impl DirectoryColorAddPicker {
    pub(super) fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(
                tr("settings.directory_color.add_directory_color"),
                SecondaryTheme,
            )
        ...
        let dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.set_menu_header_to_static(tr("settings.directory_color.add_directory_color"));
            dropdown
        });
                        ...
                        ConstrainedBox::new(
                            Container::new(
                                Flex::row()
                                    ...
                                    .with_child(
                                        Text::new_inline(
                                            tr("settings.directory_color.add_directory"),
                                            font_family,
                                            font_size,
                                        )
