...
impl std::fmt::Display for TeamsInviteOption {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            "{}",
            match self {
                TeamsInviteOption::Link => tr("settings.teams.invite_link_tab"),
                TeamsInviteOption::Email => tr("settings.teams.invite_email_tab"),
            },
        )
    }
}
...
impl TeamsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<TeamsPageView>) -> Self {
        ...
        let team_name_placeholder = tr("settings.teams.team_name_placeholder");
        ...
        let approve_domains_block_editor = ctx.add_typed_action_view(|ctx| {
            let placeholder = tr("settings.teams.domains_placeholder");
        ...
        let email_invites_block_editor = ctx.add_typed_action_view(|ctx| {
            let placeholder = tr("settings.teams.emails_placeholder");
            WordBlockEditorView::new(
                ctx,
                &placeholder,
                font_size,
        ...
        let rename_team_editor = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text(&tr("settings.teams.new_team_name_placeholder"), ctx);
            input
        });
        ...
        let transfer_ownership_modal = ctx.add_typed_action_view(|ctx| {
            Modal::new(
                Some(tr("settings.teams.transfer_ownership_title")),
    ...
    fn handle_model_event(
        ...
        match event {
            ...
            UserWorkspacesEvent::EmailInviteRejected(err) => {
                self.update_team_members_state(ctx);
                self.show_error(tr("settings.teams.failed_send_invite"), Some(err), ctx)
            }
            ...
            UserWorkspacesEvent::ToggleInviteLinksSuccess => {
                self.show_success(tr("settings.teams.toggled_invite_links"), ctx);
                ctx.notify();
            }
            UserWorkspacesEvent::ToggleInviteLinksRejected(err) => {
                self.show_error(
                    tr("settings.teams.failed_toggle_invite_links"),
                    Some(err),
                    ctx,
                );
            }
            UserWorkspacesEvent::ResetInviteLinks => {
                self.show_success(tr("settings.teams.reset_invite_links_success"), ctx);
                ctx.notify();
            }
            UserWorkspacesEvent::ResetInviteLinksRejected(err) => {
                self.show_error(
                    tr("settings.teams.failed_reset_invite_links"),
                    Some(err),
                    ctx,
                );
            }
            UserWorkspacesEvent::DeleteTeamInvite => {
                self.update_team_members_state(ctx);
                self.show_success(tr("settings.teams.deleted_invite"), ctx);
            }
            UserWorkspacesEvent::DeleteTeamInviteRejected(err) => {
                self.show_error(tr("settings.teams.failed_delete_invite"), Some(err), ctx);
            }
            ...
            UserWorkspacesEvent::AddDomainRestrictionsRejected(err) => self.show_error(
                tr("settings.teams.failed_add_domain_restriction"),
                Some(err),
                ctx,
            ),
            ...
            UserWorkspacesEvent::DeleteDomainRestrictionRejected(err) => self.show_error(
                tr("settings.teams.failed_delete_domain_restriction"),
                Some(err),
                ctx,
            ),
            ...
            UserWorkspacesEvent::GenerateUpgradeLinkRejected(err) => self.show_error(
                tr("settings.teams.failed_generate_upgrade_link"),
                Some(err),
                ctx,
            ),
            ...
            UserWorkspacesEvent::GenerateStripeBillingPortalLinkRejected(err) => self.show_error(
                tr("settings.teams.failed_generate_billing_link"),
                Some(err),
                ctx,
            ),
            UserWorkspacesEvent::ToggleTeamDiscoverabilitySuccess => {
                self.show_success(tr("settings.teams.toggled_team_discoverability"), ctx);
                ctx.notify();
            }
            UserWorkspacesEvent::ToggleTeamDiscoverabilityRejected(err) => {
                self.show_error(
                    tr("settings.teams.failed_toggle_team_discoverability"),
                    Some(err),
                    ctx,
                );
            }
            UserWorkspacesEvent::JoinTeamWithTeamDiscoverySuccess => {
                ...
                let message = self
                    ...
                    .as_ref(ctx)
                    .current_team()
                    .map_or(tr("settings.teams.joined_team"), |team| {
                        tr_with("settings.teams.joined_named_team", &[("team", &team.name)])
                    });
                self.show_success(message, ctx);
                ctx.notify();
            }
            UserWorkspacesEvent::JoinTeamWithTeamDiscoveryRejected(err) => {
                self.show_error(tr("settings.teams.failed_join_team"), Some(err), ctx);
            }
            ...
            UserWorkspacesEvent::TransferTeamOwnershipSuccess => {
                self.show_success(tr("settings.teams.transferred_ownership_success"), ctx);
                ctx.notify();
            }
            UserWorkspacesEvent::TransferTeamOwnershipRejected(err) => {
                self.show_error(
                    tr("settings.teams.failed_transfer_ownership"),
                    Some(err),
                    ctx,
                );
            }
            UserWorkspacesEvent::SetTeamMemberRoleSuccess => {
                self.update_team_members_state(ctx);
                self.show_success(tr("settings.teams.updated_member_role_success"), ctx);
            }
            UserWorkspacesEvent::SetTeamMemberRoleRejected(err) => {
                self.show_error(
                    tr("settings.teams.failed_update_member_role"),
                    Some(err),
                    ctx,
                );
            }
    ...
    fn handle_team_update_event(
        ...
        match event {
            TeamUpdateManagerEvent::LeaveError => {
                let error = tr("settings.teams.error_leaving_team");
                self.show_error(error, None, ctx);
            }
            TeamUpdateManagerEvent::LeaveSuccess => {
                self.show_success(tr("settings.teams.left_team_success"), ctx);
                ctx.notify();
            }
            TeamUpdateManagerEvent::RenameTeamSuccess => {
                self.show_success(tr("settings.teams.renamed_team_success"), ctx)
            }
            TeamUpdateManagerEvent::RenameTeamError => {
                self.show_error(tr("settings.teams.failed_rename_team"), None, ctx)
            }
        }
    }
    ...
    fn copy_invite_link(&mut self, link: &str, ctx: &mut ViewContext<Self>) {
        ...
        self.show_toast(tr("settings.teams.link_copied"), ToastFlavor::Default, ctx);
    }
    ...
    fn send_email_invites(&mut self, team_uid: ServerId, ctx: &mut ViewContext<Self>) {
        ...
        let message = if unique_emails.len() == 1 {
            tr("settings.teams.invite_on_the_way")
        } else {
            tr_with(
    ...
    fn team_to_item_list(&self, team: &Team, current_user_email: &str) -> Vec<Item> {
        ...
        team.pending_email_invites.iter().for_each(|email_invite| {
            ...
            let actions = if current_user_has_admin_permissions {
                vec![ItemAction {
                    icon: Icon::X,
                    label: tr("settings.teams.cancel_invite"),
            ...
            if member.email != *current_user_email {
                ...
                if current_user_has_owner_permissions && !team_member_has_owner_permissions {
                    actions.push(ItemAction {
                        icon: Icon::Users,
                        label: tr("settings.teams.transfer_ownership"),
                    ...
                    && !team_member_has_owner_permissions
                {
                    if team_member_has_admin_permissions {
                        actions.push(ItemAction {
                            icon: Icon::ArrowDown,
                            label: tr("settings.teams.demote_from_admin"),
                        ...
                        actions.push(ItemAction {
                            icon: Icon::ArrowUp,
                            label: tr("settings.teams.promote_to_admin"),
                            action: TeamsPageAction::SetTeamMemberRole {
                                team_uid: team.uid,
                                user_uid: member.uid,
                ...
                if current_user_has_admin_permissions && !team_member_has_owner_permissions {
                    actions.push(ItemAction {
                        icon: Icon::X,
                        label: tr("settings.teams.remove_from_team"),
...
impl TeamsWidget {
    fn customer_type_label(customer_type: CustomerType) -> String {
        match customer_type {
            CustomerType::Free => tr("common.free"),
            CustomerType::Turbo => tr("settings.teams.plan_turbo"),
            CustomerType::SelfServe => tr("settings.teams.plan_team"),
            CustomerType::Prosumer => tr("settings.teams.plan_pro"),
            CustomerType::Legacy => tr("settings.teams.plan_early_adopter"),
            CustomerType::Enterprise => tr("settings.teams.plan_enterprise"),
            CustomerType::Business => tr("settings.teams.plan_business"),
            CustomerType::Lightspeed => tr("settings.teams.plan_lightspeed"),
            CustomerType::Build => tr("settings.teams.plan_build"),
            CustomerType::BuildMax => tr("settings.teams.plan_max"),
            CustomerType::Unknown => String::new(),
        }
    }
    ...
    fn render_grow_team_warning_alert(
        ...
        let title = match warning {
            GrowTeamWarning::SeatCapReached => tr("settings.teams.warning_team_full_title"),
            GrowTeamWarning::SeatCapExceeded => {
                tr("settings.teams.warning_member_limit_exceeded_title")
            }
            GrowTeamWarning::PaymentPastDue => tr("settings.teams.warning_payment_past_due_title"),
            GrowTeamWarning::PaymentUnpaid => {
                tr("settings.teams.warning_subscription_unpaid_title")
            }
        };
        ...
        let body_prefix = match warning {
            GrowTeamWarning::SeatCapReached => tr("settings.teams.warning_team_full_body"),
            GrowTeamWarning::SeatCapExceeded => {
                tr("settings.teams.warning_member_limit_exceeded_body")
            }
            GrowTeamWarning::PaymentPastDue => tr("settings.teams.warning_payment_past_due_body"),
            GrowTeamWarning::PaymentUnpaid => tr("settings.teams.warning_subscription_unpaid_body"),
        };
        ...
        let cta_sentence = if !has_admin_permissions {
            if is_delinquency {
                tr("settings.teams.warning_contact_admin_restore")
            } else {
                tr("settings.teams.warning_contact_admin_grow")
            }
        } else {
            match cta {
                GrowTeamWarningCta::Upgrade => tr("settings.teams.warning_upgrade_grow"),
                GrowTeamWarningCta::UpdateBilling => {
                    tr("settings.teams.warning_update_payment_restore")
                }
                GrowTeamWarningCta::ContactSupport => {
                    tr("settings.teams.warning_contact_support_restore")
                }
                GrowTeamWarningCta::None => {
                    if is_delinquency {
                        tr("settings.teams.warning_contact_support_restore")
                    } else {
                        tr("settings.teams.warning_contact_sales_grow")
                    }
                }
            }
        };
    ...
    fn outgrow_upgrade_line_copy(billing_metadata: &BillingMetadata) -> (String, String) {
        if billing_metadata.customer_type == CustomerType::Business {
            (
                tr("settings.teams.upgrade_to_enterprise"),
                tr("settings.teams.unlimited_team_member_limit_suffix"),
            )
        } else {
            (
                tr("settings.teams.upgrade_to_business"),
                tr("settings.teams.higher_team_member_limit_suffix"),
            )
        }
    }
    ...
    fn render_team_member_cost_info(
        ...
        let prorated_message = if has_admin_permissions {
            tr("settings.teams.prorated_current_admin")
        } else {
            tr("settings.teams.prorated_non_admin")
        };
        ...
        {
            tr_with(
                "settings.teams.additional_members_cost_known",
                &[
            ...
            tr_with(
                "settings.teams.additional_members_cost_unknown",
                &[("prorated_message", &prorated_message)],
            )
        };
        ...
        if team.billing_metadata.customer_type != CustomerType::Unknown {
            left_side.add_child(
                Container::new(render_customer_type_badge(
                    appearance,
                    Self::customer_type_label(team.billing_metadata.customer_type),
                ))
                .with_margin_left(12.)
                .finish(),
            );
        ...
        match team.billing_metadata.delinquency_status {
            DelinquencyStatus::PastDue => {
                left_side.add_child(
                    Container::new(self.render_delinquency_badge(
                        appearance,
                        tr("settings.teams.past_due_badge"),
            ...
            DelinquencyStatus::Unpaid => {
                left_side.add_child(
                    Container::new(self.render_delinquency_badge(
                        appearance,
                        tr("settings.teams.unpaid_badge"),
    ...
    fn render_manage_billing_button(
        ...
        appearance
            ...
            .with_text_and_icon_label(
                TextAndIcon::new(
                    TextAndIconAlignment::IconFirst,
                    tr("settings.teams.manage_billing"),
    ...
    fn render_admin_panel_button(
        ...
        appearance
            ...
            .with_text_and_icon_label(
                TextAndIcon::new(
                    TextAndIconAlignment::IconFirst,
                    tr("settings.teams.open_admin_panel"),
        ...
        if team.billing_metadata.customer_type == CustomerType::Enterprise {
            ...
            if team.billing_metadata.can_upgrade_to_higher_tier_plan() {
                let description = if team.billing_metadata.can_upgrade_to_build_plan() {
                    tr("settings.teams.upgrade_to_build")
                } else {
                    match team.billing_metadata.customer_type {
                        CustomerType::Prosumer => tr("settings.teams.upgrade_to_turbo_plan"),
                        CustomerType::Turbo => tr("settings.teams.upgrade_to_lightspeed_plan"),
    ...
    fn render_plan_usage(
        ...
        let sub_header_text = match team.billing_metadata.customer_type {
            CustomerType::Free => tr("settings.teams.free_plan_usage_limits"),
            _ => tr("settings.teams.plan_usage_limits"),
        };
            ...
            if !policy.is_unlimited {
                ...
                shared_notebooks_column.add_child(
                    self.render_plan_usage_header(
                        tr("settings.teams.shared_notebooks"),
                        appearance,
                    ),
                );
        ...
        if let Some(policy) = team.billing_metadata.tier.shared_workflows_policy {
            if !policy.is_unlimited {
                ...
                shared_workflows_column.add_child(
                    self.render_plan_usage_header(
                        tr("settings.teams.shared_workflows"),
                        appearance,
                    ),
                );
    ...
    fn render_team_invitation_section(
        ...
        invitation_section.add_child(
            Container::new(
                self.render_subsection_header(tr("settings.teams.invite_team_members"), appearance),
            )
    ...
    fn render_invite_by_link_section(
        ...
        let header = self.render_subsubsection_header(tr("settings.teams.by_link"), appearance);
        let text_column = if has_admin_permissions {
            Flex::column()
                .with_child(header)
                .with_child(
                    Container::new(self.render_sub_text(
                        tr("settings.teams.invite_link_toggle_instructions"),
                        appearance,
    ...
    fn render_invite_by_email_section(
        ...
        section.add_child(
            Container::new(
                self.render_subsubsection_header(tr("settings.teams.by_email"), appearance),
            )
            .with_padding_top(CONTENT_SEPARATION_PADDING)
        ...
        section.add_child(
            Container::new(self.render_sub_text(
                tr("settings.teams.invite_by_email_expiry_instructions"),
                appearance,
                Some(Coords::uniform(0.).right(48.)),
            ))
            ...
            && view.email_invites_block_editor_state.num_chips > 0
        {
            section.add_child(
                Container::new(self.render_error_sub_text(
                    tr("settings.teams.invalid_emails_instructions"),
                    appearance,
                ))
                .with_padding_top(8.)
                .finish(),
    ...
    fn render_team_members_section(
        ...
        let header_row = Flex::row()
            ...
            .with_cross_axis_alignment(CrossAxisAlignment::Center)
            .with_child(
                self.render_subsection_header(tr("settings.teams.team_members"), appearance),
            )
    ...
    fn render_team_members_count(&self, team: &Team, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let count_label = if count == 1 {
            tr("settings.teams.one_team_member")
        } else {
            tr_with(
                "settings.teams.team_members_count",
        ...
        let plan_display = Self::customer_type_label(team.billing_metadata.customer_type);
        let tooltip_text = tr_with(
            "settings.teams.member_capacity_tooltip",
    ...
    fn render_outgrow_upgrade_cta(
        ...
        let prefix = self.render_sub_text(
            tr("settings.teams.need_more_seats_prefix"),
            appearance,
            None,
        );
        let link = appearance
    ...
    fn render_approved_domains_section(
        ...
        if has_admin_permissions {
            section.add_child(
                Container::new(self.render_sub_text(
                    tr("settings.teams.invite_link_domain_restrictions_instructions"),
                    appearance,
                    Some(Coords::uniform(0.).right(48.)),
                ))
                .with_padding_top(16.)
                ...
                && view.approve_domains_block_editor_state.num_chips > 0
            {
                section.add_child(
                    Container::new(self.render_error_sub_text(
                        tr("settings.teams.invalid_domains_instructions"),
                        appearance,
                    ))
                    .with_padding_top(8.)
                    .finish(),
        ...
        let domains_as_items: Vec<Item> = team
            ...
            .map(|domain_restriction| {
                let actions = if has_admin_permissions {
                    vec![ItemAction {
                        icon: Icon::X,
                        label: tr("settings.teams.remove_domain"),
    ...
    fn render_approve_domains_button(
        ...
        Container::new(self.render_button(
            &tr("settings.teams.set_domain_restrictions"),
            variant,
    ...
    fn render_discoverability_toggle_section(
        ...
        // toggle on the right.
        let header =
            self.render_subsubsection_header(tr("settings.teams.by_discovery"), appearance);

        let domain = current_user_email.split('@').nth(1).unwrap_or("");
        let team_discoverability_instructions = tr_with(
            "settings.teams.allow_domain_join_team",
            &[("domain", domain)],
        );
    ...
    fn render_leave_or_delete_team_button(
        ...
        let (label, action) = if is_team_owner {
            (
                tr("settings.teams.delete_team"),
                TeamsPageAction::ShowDeleteTeamConfirmationDialog,
            )
        } else {
            (
                tr("settings.teams.leave_team"),
                TeamsPageAction::ShowLeaveTeamConfirmationDialog,
            )
        };
                ...
                match item.state {
                    ItemState::Expired => {
                        pending_and_close_row.add_child(
                            self.render_state_chip(
                                appearance,
                                tr("settings.teams.expired_badge"),
                    ...
                    ItemState::Pending => {
                        pending_and_close_row.add_child(
                            self.render_state_chip(
                                appearance,
                                tr("settings.teams.pending_badge"),
    ...
    fn render_copy_link_row(
        ...
        let (link_text, button_enabled) = match &team_metadata.invite_code {
            ...
            None => (tr("settings.teams.failed_load_invite_link"), false),
        };
        let theme = appearance.theme();
    ...
    fn render_create_team_page(
        ...
        page.add_child(render_sub_header(
            appearance,
            tr("settings.teams.teams"),
            None,
        ));
        page.add_child(
            self.render_sub_header_with_subtext_color(appearance, tr("settings.teams.create_team")),
        );
        page.add_child(
            Container::new(
                self.render_description(tr("settings.teams.create_team_description"), appearance),
            )
            .with_padding_top(6.)
            .finish(),
        );
        ...
        if view.auth_state.is_on_work_domain().unwrap_or_default() {
            ...
            let checkbox_row_text = if let Some(domain) = view.auth_state.user_email_domain() {
                ...
                tr("settings.teams.allow_same_domain_join_team")
            };
        ...
        if !view.discoverable_teams_states.is_empty() {
            ...
            page.add_child(self.render_sub_header_with_subtext_color(
                appearance,
                tr("settings.teams.join_existing_team"),
            ));
    ...
    fn render_single_team_in_team_discovery(
        ...
        let teammate_string = if team_state.team.num_members == 1 {
            tr("settings.teams.one_teammate")
        } else {
            tr_with(
        ...
        single_team.add_child(
            Container::new(self.render_sub_text(
                tr("settings.teams.join_team_description"),
                appearance,
                None,
            ))
            .with_padding_top(12.)
    ...
    fn render_create_team_button(
        ...
        let mut button = appearance
            ...
            .with_centered_text_label(tr("common.create"))
            .with_style(UiComponentStyles {
                font_color: Some(
                    appearance
                        .theme()
    ...
    fn render_join_team_button(
        ...
        if team_state.team.team_accepting_invites {
            ...
            appearance
                ...
                .with_centered_text_label(tr("settings.teams.contact_admin_request_access"))
                .disabled()
                .build()
                .finish()
        }
...
impl SettingsWidget for TeamsWidget {
    ...
    fn render(
        ...
        let content = if NetworkStatus::as_ref(app).is_online() {
            ...
            appearance
                .ui_builder()
                .span(tr("settings.teams.offline"))
                .build()
                .finish()
        };
