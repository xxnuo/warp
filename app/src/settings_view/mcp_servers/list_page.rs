...
impl MCPServersListPageView {
    ...
    fn create_template_server_card(
        ...
        let server_card = ServerCardView::new(
            ...
            template
                .description
                .clone()
                .or_else(|| Some(tr("settings.mcp.available_to_install"))),
        ...
        match update {
            MCPServerUpdate::CloudTemplate { .. } => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::success(tr("settings.mcp.server_updated"));
    ...
    fn render_file_based_mcp_section(
        ...
        let label = render_body_item_label::<MCPServersListPageViewAction>(
            tr("settings.mcp.auto_spawn_third_party"),
            None,
            None,
        ...
        let description_fragments = vec![
            FormattedTextFragment::plain_text(tr("settings.mcp.auto_spawn_description")),
            FormattedTextFragment::hyperlink(
                tr("settings.mcp.see_supported_providers"),
    ...
    fn render_page_body(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let description_fragments = vec![
            FormattedTextFragment::plain_text(tr("settings.mcp.description")),
            FormattedTextFragment::hyperlink(
                ...
                && filtered_file_based_cards.is_empty()
            {
                ...
                if !shared_server_cards.is_empty() {
                    ...
                    let shared_by_text = match team_name {
                        Some(name) => tr_with(
                            "settings.mcp.shared_by_warp_and_team",
                            &[("team", name.as_str())],
                        ),
                        None => tr("settings.mcp.shared_by_warp_and_devices"),
                    };
                    ...
                    page.add_child(self.render_server_cards_section(
                        &tr("settings.mcp.shared_from_warp"),
                ...
                for (provider, cards) in &filtered_file_based_cards {
                    let section_title = tr_with(
                        "settings.mcp.detected_from_provider",
                ...
                Align::new(
                    Flex::column()
                        ...
                        .with_child(
                            appearance
                                .ui_builder()
                                .wrappable_text(tr("settings.mcp.empty_state"), true)
                ...
                Align::new(
                    Flex::column()
                        ...
                        .with_child(
                            appearance
                                .ui_builder()
                                .wrappable_text(tr("settings.mcp.no_search_results"), true)
                                .with_style(style::description_text(appearance))
                                .build()
                                .finish(),
                        )
        ...
        for provider in MCPProvider::iter() {
            ...
            let server_card = ServerCardView::new(
                ...
                installation
                    ...
                    .description
                    .clone()
                    .or_else(|| Some(tr("settings.mcp.detected_from_config_file"))),
    ...
    fn get_title_chip_text(
        ...
        match item_id {
            ...
            | ServerCardItemId::TemplatableMCPInstallation(_) => {
                ...
                if is_shared {
                    match creator {
                        Some(creator) => Some(TitleChip::text(tr_with(
                            "settings.mcp.shared_by_creator",
                            &[("creator", creator.as_str())],
                        ))),
                        None => Some(TitleChip::text(tr("settings.mcp.shared_by_team_member"))),
                    }
                } else if matches!(item_id, ServerCardItemId::TemplatableMCP(_)) {
                    Some(TitleChip::text(tr("settings.mcp.from_another_device")))
                } else {
                    None
                }
            }
