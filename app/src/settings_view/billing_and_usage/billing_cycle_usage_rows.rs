...
impl SourceFilter {
    pub fn label(self) -> String {
        match self {
            SourceFilter::All => tr("common.all"),
            SourceFilter::Local => tr("settings.billing.source.local"),
            SourceFilter::Cloud => tr("settings.billing.source.cloud"),
        }
    }
...
fn viewer_identity(app: &AppContext) -> (Option<String>, String) {
    ...
    let display_name = auth_state
        ...
        .unwrap_or_else(|| tr("settings.billing.your_usage"));
...
impl MemberUsageRow {
    ...
    fn for_other_members(entries: &[BillingCycleUsageEntry]) -> Self {
        ...
        Self {
            ...
            subject_uid: None,
            display_name: tr("settings.billing.other_members"),
            total_credits,
            total_cost_cents,
            segments,
            bar_max_credits: total_credits.max(1),
...
fn render_service_account_info_tooltip(appearance: &Appearance) -> Box<dyn Element> {
    ...
    let text = Text::new_inline(
        tr("settings.billing.automated_agent_tooltip"),
...
fn render_member_header(
    ...
    let subheader = render_section_subheader(&tr("settings.billing.members"), appearance);
