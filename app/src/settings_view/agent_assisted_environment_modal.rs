...
impl AgentAssistedEnvironmentModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let add_repo_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(tr("settings.environments.add_repo"), SecondaryTheme)
        ...
        let create_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(tr("settings.environments.create_environment"), PrimaryTheme)
                .on_click(|ctx| {
    ...
    fn render_selected_section(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        col.add_child(
            self.render_section_title(&tr("settings.agent_env.selected_repos"), appearance),
        );

        if self.selected_repo_paths.is_empty() {
            col.add_child(
                Text::new(
                    tr("settings.agent_env.no_repos_selected"),
                    appearance.ui_font_family(),
    ...
    fn render_available_section(&self, appearance: &Appearance) -> Box<dyn Element> {
            ...
            .with_child(
                Expanded::new(
                    ...
                    self.render_section_title(
                        &tr("settings.agent_env.available_indexed_repos"),
                        appearance,
                    ),
                )
                .finish(),
        ...
        if self.available_repos.is_empty() {
            let text = if cfg!(all(feature = "local_fs", not(target_family = "wasm"))) {
                if self.available_repos_loading {
                    tr("settings.agent_env.loading_indexed_repos")
                } else {
                    tr("settings.agent_env.no_indexed_repos")
                }
            } else {
                tr("settings.agent_env.local_repo_unavailable")
            };
        ...
        if !has_any_available {
            col.add_child(
                Text::new(
                    tr("settings.agent_env.all_indexed_repos_selected"),
                    appearance.ui_font_family(),
                    appearance.ui_font_size() * 0.95,
                )
                .with_color(theme.nonactive_ui_text_color().into())
    ...
    fn show_not_a_repo_toast(&self, selected_path: &Path, ctx: &mut ViewContext<Self>) {
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            let toast = DismissibleToast::error(tr_with(
                "settings.agent_env.not_git_repo",
                &[("path", path.as_str())],
            ))
            ...
            move |paths_result, ctx| {
                let result = paths_result.and_then(|paths| {
                    paths.into_iter().next().map(PathBuf::from).ok_or_else(|| {
                        FilePickerError::DialogFailed(tr(
                            "settings.agent_env.no_directory_selected",
                        ))
                    })
                });
    ...
    fn render_dialog(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let description = if FeatureFlag::FullSourceCodeEmbedding.is_enabled() {
            tr("settings.agent_env.description_indexed")
        } else {
            tr("settings.agent_env.description")
        };
