...
impl KeybindingRow {
    ...
    fn render_clicked(
        ...
        let conflict_warning = if has_conflicting_binding {
            render_text(
                &tr("settings.keybindings.shortcut_conflict_warning"),
        ...
        let press_new_shortcut_text = render_text(
            &tr("settings.keybindings.press_new_shortcut"),
            None,
            appearance,
        );
    ...
    fn get_edit_button_row(&self, appearance: &Appearance, index: usize) -> Box<dyn Element> {
        ...
        let clear = Container::new(
            Hoverable::new(
                ...
                |state| {
                    render_button(
                        tr("settings.keybindings.default"),
                        appearance,
                        self.get_button_text_color(appearance, state),
                    )
                },
        ...
        let cancel = Container::new(
            Hoverable::new(
                ...
                |state| {
                    ...
                    if index == 0 {
                        SavePosition::new(
                            render_button(tr("common.cancel"), appearance, cancel_button_color),
...
impl KeybindingsWidget {
    fn render_description(
        ...
        let mut description = Flex::column().with_child(render_text(
            &tr("settings.keybindings.description"),
            Some(UiComponentStyles {
                ...
                Wrap::row()
                    .with_child(
                        Container::new(render_text(
                            &tr("settings.keybindings.use_shortcut_prefix"),
                            Some(UiComponentStyles {
                                font_size: Some(font_size),
                                font_color: Some(
                                    appearance
                    ...
                    .with_child(
                        Container::new(render_text(
                            &tr("settings.keybindings.use_shortcut_suffix"),
                            Some(UiComponentStyles {
                                font_size: Some(font_size),
                                font_color: Some(
                                    appearance
    ...
    fn render(
        ...
        let local_only_icon_state = if *CloudPreferencesSettings::as_ref(app).settings_sync_enabled
        {
            Some(LocalOnlyIconState::Visible {
                ...
                custom_tooltip: Some(tr("settings.keybindings.not_synced_to_cloud")),
            })
        } else {
            None
        };
        ...
        let subheader = render_sub_header(
            appearance,
            tr("settings.keybindings.configure"),
        ...
        Flex::column()
            ...
            .with_child(render_columns(
                Container::new(render_text(
                    &tr("settings.keybindings.command"),
                    Some(UiComponentStyles {
