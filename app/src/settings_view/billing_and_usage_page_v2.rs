...
impl BillingAndUsagePageV2View {
    ...
    fn render_plan_section(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        if let Some(team) = workspaces.current_team() {
            ...
            right_side.add_child(
                Container::new(render_customer_type_badge(appearance, tr("common.free")))
                    .with_margin_right(8.)
                    .finish(),
            );
            right_side.add_child(
            ...
            cards_row.add_child(
                Expanded::new(
                    ...
                    render_balance_card(
                        ...
                        &tr("settings.billing.base_credits"),
                        &reset_str,
                        base_remaining,
                        base_limit,
                        outline_color,
            ...
            cards_row.add_child(
                Expanded::new(
                    ...
                    render_balance_card(
                        ...
                        &tr("settings.billing.personal_credits"),
            ...
            cards_row.add_child(
                Expanded::new(
                    ...
                    render_balance_card(
                        appearance,
                        BONUS_CREDITS_DOT_COLOR,
                        &tr("settings.billing.team_credits"),
    ...
    fn addon_credits_panel_state(
            ...
            if !has_admin_permissions {
                ...
                return AddonCreditsPanelState::IneligiblePlan(
                    AddonCreditsRestriction::UpgradeToBuild {
                        link_text: tr("settings.billing.upgrade_to_build_plan"),
        ...
        let description_text = if team_count > 1 {
            tr("settings.billing.addon_credits_description_personal")
        } else {
        ...
        let price_label = selected_credit_option
            .map(|opt| {
                ...
                tr_with(
                    "settings.billing.credits_price",
        ...
        let auto_reload_credit_amount = selected_credit_option
            .map(|o| {
                tr_with(
                    "settings.billing.credits_amount",
                    &[("credits", &o.credits.separate_with_commas())],
                )
            })
            .unwrap_or_else(|| tr("settings.billing.selected_credit_amount"));
        let auto_reload_tooltip_text = tr_with(
            "settings.billing.auto_reload_team_tooltip",
            &[("amount", auto_reload_credit_amount.as_str())],
        );
        let warning_text = if delinquent && has_admin_permissions {
            Some(tr("settings.billing.addon_credits_delinquent_warning"))
        } else if delinquent {
            Some(tr(
                "settings.billing.addon_credits_non_admin_delinquent_warning",
            ))
        } else if workspace
            .billing_metadata
            .has_failed_addon_credit_auto_reload_status()
        {
            Some(if has_admin_permissions {
                ...
                tr("settings.billing.restricted_billing_usage_non_admin_warning")
            })
        } else if would_exceed {
            Some(match (auto_reload_enabled, has_admin_permissions) {
                (true, true) => tr("settings.billing.auto_reload_paused_exceeds_limit"),
                (true, false) => tr("settings.billing.auto_reload_paused_team_exceeds_limit"),
                (false, true) => tr("settings.billing.purchase_exceeds_limit"),
                (false, false) => tr("settings.billing.purchase_team_exceeds_limit"),
            })
        } else {
            None
        };
        ...
        if !has_admin_permissions && auto_reload_enabled {
            ...
            let description_text = match configured_auto_reload_option {
                Some(option) => {
                    ...
                    tr_with(
                        "settings.billing.auto_reload_non_admin_description_with_price",
                        &[("credits", &credits), ("price", &price)],
                    )
                }
                None => tr("settings.billing.auto_reload_non_admin_description"),
            };
    ...
    fn render_addon_credits_ineligible_plan_card(
        ...
        let explanation = match restriction {
                ...
                FormattedTextElement::new(
                    FormattedText::new([FormattedTextLine::Line(vec![
                        ...
                        FormattedTextFragment::plain_text(tr(
                            "settings.billing.purchase_addon_suffix",
                        )),
                    ])]),
            ...
            AddonCreditsRestriction::ContactAccountExecutive => appearance
                .ui_builder()
                .paragraph(tr("settings.billing.contact_account_executive_addon"))
                .with_style(UiComponentStyles {
            ...
            AddonCreditsRestriction::ContactTeamAdmin => appearance
                .ui_builder()
                .paragraph(tr("settings.billing.contact_team_admin_enable_addon"))
                .with_style(UiComponentStyles {
                    font_color: Some(theme.sub_text_color(bg).into()),
                    ..Default::default()
                })
        ...
        let header = Text::new_inline(
            tr("settings.billing.buy_credits"),
            appearance.ui_font_family(),
            HEADER_FONT_SIZE,
        )
    ...
    fn render_addon_credits_non_admin_auto_reload_card(
        ...
        let auto_reload_header = Text::new_inline(
            tr("settings.billing.auto_reload_enabled"),
            appearance.ui_font_family(),
            HEADER_FONT_SIZE,
        )
        .with_color(theme.foreground().into())
    ...
    fn render_addon_credits_lower_section(
        ...
        let purchase_button_label = if self.addon_credits.purchase_loading {
            ...
            tr("settings.billing.one_time_purchase")
        };
            ...
            BillingAndUsagePageAction::UpdateAutoReloadEnabled { team_uid, enabled } => {
                let auto_reload_denomination_credits = if *enabled {
                    let Some(option) = self
                        ...
                        self.show_toast(
                            &tr("settings.billing.auto_reload_pricing_options_loading"),
                            ToastFlavor::Error,
                            ctx,
                        );
                        return;
                ...
                self.pending_auto_reload_toast = Some(if *enabled {
                    ...
                    tr("settings.billing.auto_reload_disabled_toast")
                });
                UserWorkspaces::handle(ctx).update(ctx, |ws, ctx| {
                    ws.update_addon_credits_settings(
                        *team_uid,
