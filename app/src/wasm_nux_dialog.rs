...
impl View for WasmNUXDialog {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let dialog = if self.requested_download {
            Dialog::new(
                tr("wasm_nux.open_desktop_title"),
                Some(tr("wasm_nux.future_links_open_desktop")),
                dialog_styles,
            )
            .with_bottom_row_child(Self::render_dialog_button(
                tr("wasm_nux.open_in_warp"),
            ...
            Dialog::new(tr("wasm_nux.download_title"), None, dialog_styles)
                .with_child(
                    Flex::column()
                        ...
                        .with_child(
                            appearance
                                .ui_builder()
                                .span(tr("wasm_nux.intelligent_terminal"))
            ...
            let object_kind = match web_intent_parser::current_web_intent() {
                Some(WebIntent::DriveObject(_)) => tr("wasm_nux.object_kind.drive_objects"),
                Some(WebIntent::SessionView(_)) => tr("wasm_nux.object_kind.shared_sessions"),
                _ => tr("wasm_nux.object_kind.warp_links"),
            };

            Dialog::new(
                tr_with(
                    "wasm_nux.always_open_on_web",
                    &[("object_kind", &object_kind)],
                ),
                Some(tr("wasm_nux.change_settings_anytime")),
                dialog_styles,
            )
            .with_bottom_row_child(Self::render_dialog_button(
                tr("common.yes"),
