...
impl DeleteConversationConfirmationDialog {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let delete_button = ctx.add_typed_action_view(|ctx| {
            ActionButton::new(tr("common.delete"), DangerPrimaryTheme)
        ...
        let title = self
            ...
            .map(|s| {
                tr_with(
                    ...
                    &[("name", &s.conversation_title)],
                )
            })
            .unwrap_or_else(|| tr("workspace.delete_conversation.title"));

        let dialog = Dialog::new(
            title,
            Some(tr("workspace.delete_conversation.description")),
            UiComponentStyles {
