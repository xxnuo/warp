...
use crate::search::{query_filter_placeholder_text, QueryFilter};
...
impl<T: Action + Clone> SearchBar<T> {
    ...
    fn emit_accessibility_content(&self, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(selected_result) = self.state.as_ref(ctx).selected_result() {
            ...
            let a11y_content_text = tr_with("search.a11y.selected", &[("label", &label)]);
        ...
        self.editor_handle.update(ctx, |editor, ctx| {
            if editor.is_empty(ctx) {
                ...
                match self.state.as_ref(ctx).query_filter {
                    Some(filter) => {
                        editor.set_placeholder_text(query_filter_placeholder_text(filter), ctx);
                    }
                    None => {
                        editor.set_placeholder_text(self.placeholder_text.as_str(), ctx);
                    }
                }
            }
        });
