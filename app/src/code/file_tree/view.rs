...
impl FileTreeView {
    ...
    fn context_menu_items(
        ...
        if is_remote {
            ...
            match item {
                    ...
                    if !is_file_content_binary(&path_local) {
                        items.extend([
                            MenuItemFields::new(tr(
                                "agent_view.orchestration_pill.open_in_new_pane",
                            ))
                            .with_on_select_action(FileTreeAction::OpenInNewPane { id: id.clone() })
                            .into_item(),
                            MenuItemFields::new(tr(
                                "agent_view.orchestration_pill.open_in_new_tab",
                            ))
                        ...
                        items.push(
                            MenuItemFields::new(tr("common.open_file"))
                ...
                FileTreeItem::DirectoryHeader { .. } => {
                    items.push(
                        MenuItemFields::new(tr("code.file_tree.new_file"))
                    ...
                    if self.has_terminal_session {
                        items.push(
                            MenuItemFields::new(tr("code.file_tree.cd_to_directory"))
                    ...
                    items.push(
                        MenuItemFields::new(tr("agent_view.orchestration_pill.open_in_new_tab"))
                            .with_on_select_action(FileTreeAction::OpenInNewTab { id: id.clone() })
                            .into_item(),
                    );
                }
            ...
            let open_text = if cfg!(target_os = "macos") {
                tr("code.menu.reveal_in_finder")
            } else if cfg!(target_os = "windows") {
                tr("code.menu.reveal_in_explorer")
            } else {
                tr("code.menu.reveal_in_file_manager")
            };
            items.push(
            ...
            if !is_repo_root_dir {
                ...
                items.push(
                    MenuItemFields::new(tr("common.delete"))
        ...
        if self.has_terminal_session {
            ...
            items.push(
                MenuItemFields::new(tr("code.file_tree.attach_as_context"))
        ...
        items.extend([
            MenuItemFields::new(tr("terminal.context_menu.copy_path"))
                ...
                .into_item(),
            MenuItemFields::new(tr("code.file_tree.copy_relative_path"))
    ...
    fn render_error_state(&self, text: String, app: &AppContext) -> Box<dyn Element> {
        ...
        let main_column = Flex::column()
            ...
            .with_child(
                Text::new(
                    tr("code.file_tree.project_explorer_unavailable"),
                    appearance.ui_font_family(),
...
impl View for FileTreeView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        self.render_error_state(tr("code.file_tree.remote_unsupported"), app)
    }

    #[cfg(feature = "local_fs")]
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        if matches!(self.enablement, CodingPanelEnablementState::Disabled) {
            return self.render_error_state(tr("code.file_tree.disabled"), app);
        }
        ...
        if self.displayed_directories.is_empty() {
            ...
            if matches!(
                ...
                return self.render_error_state(tr("code.file_tree.wsl_unsupported"), app);
            }
