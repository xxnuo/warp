...
impl LoginSlideView {
    ...
    pub fn new(
        ...
        let auth_token_input = ctx.add_typed_action_view(|ctx| {
            ...
            let placeholder = tr("login.browser_open.auth_token_placeholder");
    ...
    fn privacy_disclaimer_prefix(&self) -> String {
        if self.ai_enabled {
            tr("login.select_auth.privacy_prefix_ai")
        } else {
            tr("login.select_auth.privacy_prefix_terminal")
        }
    }
    ...
    fn render_select_auth_content(&self, appearance: &Appearance) -> Vec<Box<dyn Element>> {
        ...
        let title_text = if is_terminal {
            tr("login.select_auth.title_terminal")
        } else {
            tr("login.select_auth.title_ai")
        };
        ...
        let subtitle_text = if is_terminal {
            tr("login.select_auth.subtitle_terminal")
        } else {
            tr("login.select_auth.subtitle_ai")
        };
        let subtitle =
    ...
    fn render_select_auth_bottom_nav(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let skip_label = if matches!(self.intention, OnboardingIntention::Terminal) {
            tr("login.select_auth.disable_warp_drive")
        } else {
            tr("login.select_auth.disable_ai_features")
        };
        ...
        let hint = Flex::column()
            .with_child(
                Flex::row()
                    ...
                    .with_child(
                        ui_builder
                            .span(tr("login.browser_open.middle"))
                            .with_style(sub_text_styles)
                            .build()
                            .finish(),
                    )
            ...
            .with_child(
                ui_builder
                    .span(tr("login.browser_open.suffix"))
                    .with_style(sub_text_styles)
                    .build()
                    .finish(),
            )
    ...
    fn render_privacy_settings_content(
        ...
        let title = FormattedTextElement::from_str(
            tr("login.privacy_settings.title"),
            appearance.ui_font_family(),
            36.,
        )
        .with_color(internal_colors::text_main(
    ...
    fn render_skip_dialog(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let is_terminal = matches!(self.intention, OnboardingIntention::Terminal);
        let title_text = if is_terminal {
            tr("login.skip_dialog.title_terminal")
        } else {
            tr("login.skip_dialog.title_ai")
        };
        ...
        let body_text_str = if is_terminal {
            tr("login.skip_dialog.body_terminal")
        } else {
            tr("login.skip_dialog.body_ai")
        };
        let body_text =
        ...
        for &item in feature_items {
            let icon_el = ConstrainedBox::new(Icon::X.to_warpui_icon(feature_x_fill).finish())
                ...
                .finish();
            let text_el =
                FormattedTextElement::from_str(tr(item), appearance.ui_font_family(), 14.)
        ...
        let cancel_label = if is_terminal {
            tr("login.skip_dialog.enable_warp_drive")
        } else {
            tr("login.skip_dialog.enable_ai_features")
        };
        ...
        let skip_confirm_button = self.dialog_skip_button.render(
            appearance,
            button::Params {
                content: button::Content::Label(tr("common.skip_for_now").into()),
                theme: &button::themes::Primary,
                options: button::Options {
