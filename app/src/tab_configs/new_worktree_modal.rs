...
const INVALID_BRANCH_NAME_ERROR_KEY: &str = "tab_configs.invalid_branch_name";
...
impl View for NewWorktreeModal {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        body.add_child(Self::render_section_label(
            &tr("tab_configs.select_repository"),
            appearance,
        ));
        ...
        let checkbox_row = Flex::row()
            ...
            .with_child(
                Text::new_inline(
                    tr("tab_configs.autogenerate_worktree_branch_name"),
                    appearance.ui_font_family(),
                    appearance.ui_font_size(),
                )
        ...
        if !self.autogenerate_branch_name {
            body.add_child(
                Container::new(Self::render_section_label(
                    &tr("tab_configs.worktree_branch_name"),
                    appearance,
                ))
                .with_margin_top(SECTION_GAP)
                .finish(),
            ...
            if worktree_name_has_error {
                body.add_child(
                    Container::new(
                        Text::new_inline(
                            tr(INVALID_BRANCH_NAME_ERROR_KEY),
                            appearance.ui_font_family(),
                            ERROR_FONT_SIZE,
                        )
        ...
        let open_button = {
            ...
            let mut builder = appearance
                ...
                .with_text_label(tr("common.open"))
                .with_style(text_button_base)
                .with_style(UiComponentStyles {
