...
impl EventLoop {
    ...
    fn start_websocket_listener_and_writer_tasks(
        ...
        ctx.background_executor()
            .spawn(async move {
                ...
                if let Err(e) = Self::write_init_shell_script(&mut sink).await {
    ...
    async fn write_init_shell_script(sink: &mut impl Sink) -> anyhow::Result<()> {
        let init_shell_script =
            init_shell_script_for_shell(super::REMOTE_TTY_SHELL, &crate::ASSETS);
