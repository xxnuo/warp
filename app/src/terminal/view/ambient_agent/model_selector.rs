...
impl ModelSelector {
    pub fn new(
        ...
        let button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("", AgentInputButtonTheme)
                ...
                .with_tooltip(tr("ambient_agent.model_selector.button_tooltip"))
                .on_click(|ctx| {
        ...
        let search_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(tr("ambient_agent.model_selector.search_placeholder"), ctx);
            editor
        });
    ...
    fn refresh_button(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let active_label = match self.active_harness(ctx) {
            Some(harness) if !matches!(harness, Harness::Oz | Harness::Unknown) => self
                ...
                .unwrap_or_else(|| tr("ambient_agent.model_selector.default")),
    ...
    fn refresh_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if items.is_empty() {
            ...
            items.push(MenuItem::Item(
                MenuItemFields::new(tr("ambient_agent.model_selector.no_results"))
    ...
    fn build_harness_menu_items(
        ...
        let default_label = tr("ambient_agent.model_selector.default");
        if query.is_empty()
