...
const AGENT_MODE_HINT_OPTIONS: &[&str] = &[
    "terminal.input.agent_hint.option_1",
    "terminal.input.agent_hint.option_2",
    "terminal.input.agent_hint.option_3",
    "terminal.input.agent_hint.option_4",
    "terminal.input.agent_hint.option_5",
    "terminal.input.agent_hint.option_6",
    "terminal.input.agent_hint.option_7",
    "terminal.input.agent_hint.option_8",
    "terminal.input.agent_hint.option_9",
    "terminal.input.agent_hint.option_10",
    "terminal.input.agent_hint.option_11",
    "terminal.input.agent_hint.option_12",
    "terminal.input.agent_hint.option_13",
    "terminal.input.agent_hint.option_14",
    "terminal.input.agent_hint.option_15",
    "terminal.input.agent_hint.option_16",
    "terminal.input.agent_hint.option_17",
    "terminal.input.agent_hint.option_18",
    "terminal.input.agent_hint.option_19",
    "terminal.input.agent_hint.option_20",
];
    ...
    pub fn placeholder_text(&self) -> Option<String> {
        match self {
            InputSuggestionsMode::UserQueryMenu {
                ...
                ..
            } => Some(tr("terminal.input.placeholder.search_queries")),
            InputSuggestionsMode::UserQueryMenu {
                action: UserQueryMenuAction::Rewind,
                ..
            } => Some(tr("terminal.input.placeholder.search_queries_to_rewind")),
            InputSuggestionsMode::ConversationMenu => {
                Some(tr("terminal.input.placeholder.search_conversations"))
            }
            InputSuggestionsMode::SkillMenu => Some(tr("terminal.input.placeholder.search_skills")),
            InputSuggestionsMode::ModelSelector => {
                Some(tr("terminal.input.placeholder.search_models"))
            }
            InputSuggestionsMode::ProfileSelector => {
                Some(tr("terminal.input.placeholder.search_profiles"))
            }
            InputSuggestionsMode::SlashCommands if FeatureFlag::AgentView.is_enabled() => {
                Some(tr("terminal.input.placeholder.search_commands"))
            }
            InputSuggestionsMode::PromptsMenu => {
                Some(tr("terminal.input.placeholder.search_prompts"))
            }
            InputSuggestionsMode::IndexedReposMenu => {
                Some(tr("terminal.input.placeholder.search_indexed_repos"))
            }
            InputSuggestionsMode::PlanMenu { .. } => {
                Some(tr("terminal.input.placeholder.search_plans"))
            }
            _ => None,
        }
    }
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_fixed_bindings(vec![
        ...
        FixedBinding::custom(
            ...
            InputAction::Up,
            tr("terminal.input.binding.show_history"),
    ...
    app.register_editable_bindings([EditableBinding::new(
        ...
        tr("terminal.input.binding.show_warp_network_log"),
    ...
    app.register_editable_bindings([EditableBinding::new(
        "input:clear_screen",
        tr("terminal.input.binding.clear_screen"),
    ...
    app.register_editable_bindings([EditableBinding::new(
        ...
        BindingDescription::new(tr("terminal.input.binding.edit_prompt")).with_custom_description(
            bindings::MAC_MENUS_CONTEXT,
            tr("terminal.input.binding.edit_prompt"),
        ),
        ...
        && !FeatureFlag::ForceClassicCompletions.is_enabled()
    {
        app.register_editable_bindings([EditableBinding::new(
            ...
            tr("terminal.input.binding.toggle_classic_completions_mode"),
    ...
    app.register_editable_bindings([
        ...
        EditableBinding::new(
            ...
            tr("terminal.input.binding.history_search"),
        ...
        EditableBinding::new(
            ...
            tr("settings.features.open_completions_menu"),
    ...
    if let Some(custom_action) = workflows::CategoriesView::custom_action() {
        app.register_editable_bindings([EditableBinding::new(
            ...
            tr("drive.object_type.workflows"),
    ...
    app.register_editable_bindings([
        EditableBinding::new(
            ...
            tr("terminal.context_menu.ai_command_search"),
        ...
        EditableBinding::new(
            ...
            tr("terminal.input.binding.new_agent_conversation"),
        ...
        EditableBinding::new(
            ...
            tr("terminal.input.binding.trigger_auto_detection"),
        ...
        EditableBinding::new(
            ...
            tr("terminal.input.binding.clear_reset_ai_context_query"),
...
impl Input {
        ...
        if let Some(ambient_agent_view_model) = ambient_agent_view_model.as_ref() {
            ctx.subscribe_to_model(ambient_agent_view_model, |me, handle, event, ctx| {
                    ...
                    event
                {
                    ...
                    let toast_message = tr_with(
                        "terminal.input.failed_prepare_cloud_handoff",
                    ...
                    ToastStack::handle(ctx).update(ctx, |ts, ctx| {
                        ts.add_ephemeral_toast(
                            DismissibleToast::error(tr(
                                "terminal.input.attached_images_removed_model_unsupported",
                            )),
                            window_id,
                            ctx,
                        );
    ...
    pub(crate) fn collect_cloud_launch_attachments(
        ...
        if !skipped_files.is_empty() {
            ...
            let message = if skipped_files.len() == 1 {
                tr_with(
                    "terminal.input.file_not_attached_too_large",
                    &[("file", skipped_files[0].as_str())],
                )
            } else {
                let count = skipped_files.len().to_string();
                tr_with(
                    "terminal.input.files_not_attached_too_large",
                    &[("count", count.as_str())],
                )
            };
            ToastStack::handle(ctx).update(ctx, |ts, ctx| {
        ...
        match event {
            InlineConversationMenuEvent::NavigateToConversation { item_id } => {
                ...
                if let Some(action) = AgentConversationsModel::resolve_open_action(
                    ...
                    ctx.emit(Event::ShowToast {
                        message: tr("terminal.input.could_not_navigate_to_conversation"),
                        flavor: ToastFlavor::Error,
                    });
                }
            }
    ...
    fn execute_skill_command(
            ...
            .active_skill_by_reference(&reference, ctx)
        {
            ...
            None => {
                let skill = reference.to_string();
                let window_id = ctx.window_id();
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    toast_stack.add_ephemeral_toast(
                        DismissibleToast::error(tr_with(
                            "terminal.input.skill_not_found",
                            &[("skill", skill.as_str())],
                        )),
                        window_id,
                        ctx,
        ...
        let Some(conversation) = history
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                let toast = DismissibleToast::default(tr(
                    "terminal.input.no_active_conversation_to_export",
                ));
        ...
        if file_exists {
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                let toast = DismissibleToast::default(tr_with(
                    "terminal.input.export_file_overwrite",
        ...
        match fs::write(&file_path, conversation_text) {
            Ok(_) => {
                ...
                ToastStack::handle(ctx).update(ctx, move |toast_stack, ctx| {
                    let toast = DismissibleToast::default(tr_with(
                        "terminal.input.conversation_exported_to",
                        &[("path", display_path.as_str())],
                    ));
                    toast_stack.add_ephemeral_toast(toast, window_id, ctx);
                });
            ...
            Err(e) => {
                ...
                let user_message = match e.kind() {
                    std::io::ErrorKind::PermissionDenied => {
                        ...
                        tr_with(
                            "terminal.input.export_permission_denied",
                            &[("path", path.as_str())],
                        )
                    }
                    std::io::ErrorKind::NotFound => {
                        let path = file_path
                            ...
                            .unwrap_or_default();
                        tr_with(
                            "terminal.input.export_directory_not_found",
                            &[("path", path.as_str())],
                        )
                    }
                    std::io::ErrorKind::AlreadyExists => {
                        let path = file_path.display().to_string();
                        tr_with(
                            "terminal.input.export_file_already_exists",
                            &[("path", path.as_str())],
                        )
                    }
                    _ => {
    ...
    pub fn remove_excess_images(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if images_removed > 0 {
            ...
            let message = if images_removed == 1 {
                tr("terminal.input.one_image_removed_limit")
            } else {
                let count = images_removed.to_string();
                tr_with(
                    "terminal.input.images_removed_limit",
                    &[("count", count.as_str())],
                )
            };
    ...
    fn agent_mode_hint_text(&mut self, app: &AppContext) -> String {
        ...
        match (
            ...
            (InputType::Shell, false) => tr("terminal.input.run_commands"),
            (InputType::Shell, true) => tr(get_stable_agent_mode_hint_text(
            ...
            (InputType::AI, _) => {
                    ...
                    .selected_conversation_status_for_hint(app)
                {
                    Some(status) if status.is_in_progress() => {
                        if is_udi_enabled {
                            tr("terminal.input.steer_running_agent")
                        } else {
                            tr("terminal.input.steer_running_agent_classic")
                        }
                    }
                    Some(_) => {
                        if is_udi_enabled {
                            tr("terminal.input.ask_follow_up")
                        } else {
                            tr("terminal.input.ask_follow_up_classic")
                        }
                    }
                    None => tr(get_stable_agent_mode_hint_text(
                        &mut self.cached_agent_mode_hint_text,
                    )),
                }
            }
    ...
    fn cli_agent_rich_input_hint_text(&self, ctx: &ViewContext<Self>) -> Cow<'static, str> {
        if self.is_locked_in_shell_mode(ctx) {
            return Cow::Owned(tr("terminal.input.run_commands"));
        }

        CLIAgentSessionsModel::as_ref(ctx)
            .session(self.terminal_view_id)
            .map(|session| match session.agent {
                CLIAgent::Unknown => Cow::Owned(tr("terminal.input.cli_agent_rich_input_hint")),
                _ => Cow::Owned(tr_with(
                    "terminal.input.enter_prompt_for_agent",
                    &[("agent", session.agent.display_name())],
                )),
            })
            .unwrap_or_else(|| Cow::Owned(tr("terminal.input.cli_agent_rich_input_hint")))
    }
    ...
    pub fn set_zero_state_hint_text(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if self.prefix_mode(ctx) == InputPrefixMode::CloudHandoff {
            ...
            let hint = if conversation_is_empty {
                tr("terminal.input.cloud_mode_hint")
            } else {
                self.handoff_compose_state
                    .as_ref(ctx)
                    .selected_environment_id()
                    ...
                    .map(|env| {
                        tr_with(
                            "terminal.input.hand_off_to_environment",
                            &[("environment", &env.model().string_model.display_name())],
                        )
                    })
                    .unwrap_or_else(|| tr("terminal.input.handoff_to_cloud"))
            };
            self.editor.update(ctx, |editor, ctx| {
                editor.set_placeholder_text(&hint, ctx);
            });
        ...
        if self.is_cloud_mode_input_v2_composing(ctx) {
            ...
            self.editor.update(ctx, |editor, ctx| {
                if show_hint {
                    editor.set_placeholder_text(&tr("terminal.input.cloud_mode_hint"), ctx);
                } else {
        ...
        self.editor.update(ctx, |editor, ctx| {
            for command in COMMAND_REGISTRY.all_commands() {
                if let Some(hint_text) = command
                    .argument
                    .as_ref()
                    .and_then(|argument| argument.localized_hint_text())
                {
        ...
        if toggled_on && AISettings::as_ref(ctx).is_any_ai_enabled(ctx) {
            if FeatureFlag::AgentMode.is_enabled() {
                ...
                self.editor.update(ctx, |editor, ctx| {
                    editor.set_placeholder_text(&tr("terminal.input.ai_command_search_hint"), ctx);
                });
            }
        } else {
            self.editor.update(ctx, |editor, ctx| {
        ...
        if let CanExecuteCommand::No(reason) = self.can_execute_command(ctx) {
            if reason.is_existing_active_command() {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    toast_stack.add_ephemeral_toast(
                        DismissibleToast::error(tr_with(
                            "terminal.input.cannot_run_command_running",
                            &[("command", truncated_command.as_str())],
                        )),
                        window_id,
                        ctx,
                    );
    ...
    fn insert_workflow_into_input(
        ...
        let mut accessibility_text = tr_with(
            "terminal.input.workflow_command_inserted",
        ...
        ctx.emit_a11y_content(AccessibilityContent::new(
            accessibility_text,
            tr("terminal.input.workflow_argument_select_help"),
            ...
            .and_then(|selected_workflow_state| {
                selected_workflow_state.more_info_view.read(ctx, |view, _| {
                    view.selected_argument().map(|argument| {
                        tr_with(
                            "terminal.input.selected_workflow_argument",
        ...
        match event {
            ...
            InputSuggestionsEvent::ConfirmAndExecuteSuggestion {
                ...
                ctx.emit_a11y_content(AccessibilityContent::new_without_help(
                    tr_with(
                        "terminal.input.executed_command",
                ...
                if *is_listening || *is_transcribing {
                    ...
                    if self.editor.as_ref(ctx).is_empty(ctx) {
                        let placeholder = if *is_listening {
                            tr("terminal.input.listening")
                        } else {
                            tr("terminal.input.transcribing")
                        };
                        self.editor.update(ctx, |editor, ctx| {
                            editor.set_placeholder_text(&placeholder, ctx);
                        });
    ...
    fn check_image_limits_for_paste(
        ...
        if excess_images > 0 {
            let (limit_name, limit_value) = if available_per_query < available_per_conversation {
                (
                    tr("terminal.input.image_limit_per_query"),
                    MAX_IMAGE_COUNT_FOR_QUERY,
                )
            } else {
                (
                    tr("terminal.input.image_limit_per_conversation"),
                    MAX_IMAGES_PER_CONVERSATION,
                )
            };
            let limit_value = limit_value.to_string();

            let message = if excess_images == 1 {
                tr_with(
                    "terminal.input.one_image_not_attached_limit",
                    &[
                        ("limit", limit_value.as_str()),
                        ("scope", limit_name.as_str()),
                    ],
                )
            } else {
                let count = excess_images.to_string();
                tr_with(
                    "terminal.input.images_not_attached_limit",
                    &[
                        ("count", count.as_str()),
                        ("limit", limit_value.as_str()),
                        ("scope", limit_name.as_str()),
                ...
                })
            {
                if FeatureFlag::AgentHarness.is_enabled() {
                    ...
                    if !availability.has_any_enabled_harness() {
                        ...
                        ToastStack::handle(ctx).update(ctx, |ts, ctx| {
                            ts.add_ephemeral_toast(
                                DismissibleToast::error(tr(
                                    "terminal.input.no_agent_harnesses_available",
                                )),
                                window_id,
                                ctx,
                            );
                ...
                if let Some(ambient_agent_view_model) = self.ambient_agent_view_model().cloned() {
                    ...
                    if is_handoff_not_ready {
                        ...
                        ToastStack::handle(ctx).update(ctx, |ts, ctx| {
                            ts.add_ephemeral_toast(
                                DismissibleToast::default(tr(
                                    "terminal.input.preparing_handoff_try_again",
                                ))
    ...
    fn submit_ai_query(
        ...
        if shared_session_status.is_viewer() {
            if shared_session_status.is_executor() {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    toast_stack.add_ephemeral_toast(
                        DismissibleToast::error(tr("terminal.input.cannot_send_read_only_viewer")),
                        window_id,
                        ctx,
                    );
                });
        ...
        ctx.spawn(
            ...
            move |input, maybe_uploaded, ctx| {
                let Some(uploaded_files) = maybe_uploaded else {
                    ...
                    ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                        toast_stack.add_ephemeral_toast(
                            DismissibleToast::error(tr("terminal.input.too_many_attachments")),
                            window_id,
                            ctx,
                        );
                    });
        ...
        match action {
            InputAction::FocusInputBox => {
                ActionAccessibilityContent::Custom(AccessibilityContent::new(
                    tr("terminal.input.a11y_label"),
                    // TODO (a11y) use bindings from user settings
                    tr("terminal.input.a11y_helper"),
