...
impl BuyCreditsBanner {
            ...
            UserWorkspacesEvent::UpdateWorkspaceSettingsRejected(_) => {
                if self.banner_auto_reload_update_in_flight {
                    ...
                    ctx.emit(BuyCreditsBannerEvent::ShowAutoReloadError {
                        error_message: tr("terminal.buy_credits.auto_reload_error"),
                    });
                    ctx.notify();
                }
            }
    ...
    fn render_auto_reload_checkbox(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let label = Text::new_inline(
            tr("terminal.buy_credits.auto_reload"),
        ...
        let selected_credits_text = selected_credits.to_string();
        let tooltip_text = tr_with(
            "terminal.buy_credits.auto_reload_tooltip",
            &[("credits", selected_credits_text.as_str())],
        );
    ...
    fn render_auto_reload_blocked(
        ...
        let banner_description = if has_admin_permissions {
            tr("terminal.buy_credits.monthly_limit_admin_description")
        } else {
            tr("terminal.buy_credits.monthly_limit_nonadmin_description")
        };
        ...
        let banner_text = Flex::column()
            .with_children([
                appearance
                    .ui_builder()
                    .paragraph(tr("terminal.buy_credits.monthly_limit_title"))
                    .with_style(UiComponentStyles {
                        font_size: Some(14.),
                        ..Default::default()
                    })
        ...
        if has_admin_permissions {
            let manage_billing_button = appearance
                ...
                .with_text_label(tr("settings.account.manage_billing"))
                .build()
                .on_click(|ctx, _, _| {
    ...
    fn render_out_of_credits(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let make_banner_text = || {
            let mut banner_text_children = vec![appearance
                .ui_builder()
                .paragraph(tr("terminal.buy_credits.out_of_credits"))
                .with_style(UiComponentStyles {
                    font_size: Some(14.),
                    ..Default::default()
                })
            ...
            if is_at_monthly_limit || would_purchase_exceed_limit {
                ...
                let warning_text_fragments = vec![
                    FormattedTextFragment::plain_text(tr(
                        "terminal.buy_credits.purchase_exceeds_limit_prefix",
                    )),
                    FormattedTextFragment::hyperlink_action(
                        tr("terminal.buy_credits.increase_it"),
                        Action::ManageBilling,
                    ),
                    FormattedTextFragment::plain_text(tr(
                        "terminal.buy_credits.purchase_exceeds_limit_suffix",
                    )),
                ];
                ...
                let banner_description = if has_admin_permissions {
                    tr("terminal.buy_credits.add_more_credits_admin_description")
                } else {
                    tr("terminal.buy_credits.add_more_credits_nonadmin_description")
                };
        ...
        let make_buy_button = || {
            ...
            let button_text = if self.purchase_addon_credits_loading {
                tr("terminal.buy_credits.buying")
            } else {
                tr("terminal.buy_credits.buy")
            };
