...
const COMMAND_OPTION: (&str, DisplaySetting) =
    ("terminal.share_block.command", DisplaySetting::Command);
const OUTPUT_OPTION: (&str, DisplaySetting) =
    ("terminal.share_block.output", DisplaySetting::Output);
...
impl ShareBlockModal {
    pub fn new(
        ...
        let block_title_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(&tr("terminal.share_block.title_placeholder"), ctx);
            editor
        });
        ...
        let embed_display_options = [COMMAND_AND_OUTPUT_OPTION, COMMAND_OPTION, OUTPUT_OPTION]
            .map(|(name, display_setting)| (tr(name), display_setting))
            .to_vec();
    ...
    fn display_failure_toast(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.emit(ShareBlockModalEvent::ShowToast {
            message: tr("terminal.share_block.creation_failed"),
    ...
    pub fn copy(&mut self, ctx: &mut ViewContext<Self>) {
        if let Some(link) = self.link() {
            ...
            ctx.emit(ShareBlockModalEvent::ShowToast {
                message: tr("settings.shared_blocks.link_copied"),
    ...
    fn generate_embed_snippet(&self, app: &AppContext) -> Option<String> {
        ...
        if title.is_empty() {
            title = tr("terminal.share_block.default_embed_title");
        }
    ...
    pub fn copy_embed(&self, ctx: &mut ViewContext<Self>) {
        ...
        ctx.emit(ShareBlockModalEvent::ShowToast {
            message: tr("terminal.share_block.embed_code_copied"),
    ...
    fn render_create_block_buttons_row(&self, appearance: &Appearance) -> Box<dyn Element> {
        let create_link = tr("terminal.share_block.create_link");
        let get_embed = tr("terminal.share_block.get_embed");
        ...
        let text_and_icon = TextAndIcon::new(
            ...
            if let ShareRequestState::Pending(pending_share_type) = self.request_state {
                if pending_share_type == share_type {
                    tr("terminal.share_block.creating_block")
                } else {
    ...
    fn render_success_footer(
        ...
        if matches!(
            ...
            let embed_snippet = self
                ...
                .unwrap_or_else(|| tr("terminal.share_block.error_generating_embed_snippet"));
    ...
    fn render_manage_permalinks_button(&self, appearance: &Appearance) -> Box<dyn Element> {
        let mut button = appearance
            ...
            .with_centered_text_label(tr("terminal.share_block.manage_shared_blocks"))
            .with_style(
                self.button_style_overrides(appearance)
                    .set_font_size(12.)
                    .set_padding(Coords {
    ...
    fn render_modal(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let modal_title_or_block_title = Text::new_inline(
            if link_generated {
                ...
                tr("terminal.share_block.share_block")
            },
        ...
        if !link_generated {
            ...
            let show_prompt_description = appearance
                .ui_builder()
                .span(tr("terminal.share_block.show_prompt"))
                .build()
                .with_margin_left(2.)
                .finish();
        ...
        if !link_generated {
            ...
            let redact_secrets_description = appearance
                .ui_builder()
                .span(tr("terminal.share_block.redact_secrets"))
                .build()
                .with_margin_left(4.)
                .finish();
            column.add_child(
