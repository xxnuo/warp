...
impl RuleEditorView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let name_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(tr("ai.rules.editor.name_placeholder"), ctx);
            editor
        });
        ...
        let content_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(tr("ai.rules.editor.rule_placeholder"), ctx);
            editor
        });
        ...
        let save_button = ctx.add_typed_action_view(|ctx| {
            let mut button = ActionButton::new(tr("common.save"), PrimaryTheme)
        ...
        let delete_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(tr("ai.rules.editor.delete_rule"), DangerSecondaryTheme)
    ...
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        let title = if self.ai_fact.is_none() {
            tr("ai.rules.editor.add_rule")
        } else {
            tr("ai.rules.editor.edit_rule")
        };
        Container::new(
            Flex::row()
