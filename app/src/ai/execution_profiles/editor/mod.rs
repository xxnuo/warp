...
fn render_upgrade_footer(
    ...
    let upgrade = tr("common.upgrade");
    let label = format!(
        "{}{}",
        tr("settings.execution_profile.frontier_models_unavailable_prefix"),
        upgrade
    );
...
pub fn header_text() -> String {
    tr("settings.execution_profile.profile_editor")
}
    ...
    pub fn new(profile_id: ClientProfileId, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let ask_user_question_dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.set_items(
                vec![
                    DropdownItem::new(
                        tr("settings.execution_profile.never_ask"),
        ...
        let mcp_allowlist_dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.set_menu_header_to_static(tr("settings.execution_profile.select_mcp_servers"));
            dropdown
        });
        ...
        let command_allowlist_editor = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text(
                tr("settings.execution_profile.command_allowlist_placeholder"),
                ctx,
            );
            input
        });
        ...
        let command_denylist_editor = ctx.add_typed_action_view(|ctx| {
            let mut input =
                SubmittableTextInput::new(ctx).validate_on_edit(|s| Regex::new(s).is_ok());
            input.set_placeholder_text(
                tr("settings.execution_profile.command_denylist_placeholder"),
                ctx,
            );
            input
        });
        ...
        let directory_allowlist_editor = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text(
                tr("settings.execution_profile.directory_allowlist_placeholder"),
                ctx,
            );
            input
        });
        ...
        let profile_name_editor = ctx.add_view(|ctx| {
            ...
            editor.set_placeholder_text(tr("settings.execution_profile.name_placeholder"), ctx);
            editor
        });
        ...
        let delete_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(
                tr("settings.execution_profile.delete_profile"),
    ...
    fn update_profile_name_editor(
        ...
        profile_name_editor.update(ctx, |editor, ctx| {
            let display_name = if profile_data.is_default_profile {
                tr("settings.execution_profile.default")
            } else {
