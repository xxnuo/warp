...
fn blocked_action_message_for_transfer_control() -> String {
    tr("ai_block.blocked.transfer_control")
}
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_editable_bindings([EditableBinding::new(
        ...
        BindingDescription::new(tr("ai_block.take_control_running_command"))
            .with_dynamic_override(|_| Some(tr("ai_block.take_control_running_command"))),
            ...
            if !self.model.is_restored() && !error.is_invalid_api_key() {
                output_items.add_child(
                    Container::new(render_informational_footer(
                        app,
                        tr("ai_block.response_not_counted_take_over"),
                    ))
                    .with_margin_top(8.)
...
fn get_action_loading_text(action: AIAgentActionType) -> Option<String> {
    match action {
        AIAgentActionType::SearchCodebase(_) => Some(load_output_message_for_search_codebase()),
    ...
    let formatted_text = FormattedTextElement::new(
        FormattedText::new([FormattedTextLine::Line(vec![
            FormattedTextFragment::hyperlink(
                tr("ai_block.manage_agent_permissions"),
                tr("settings.ai.path"),
            ),
        ])]),
        font_size,
...
fn get_blocked_action_header(action: AIAgentActionType) -> Option<String> {
    match action {
        ...
        AIAgentActionType::SearchCodebase(..) => {
            Some(blocked_action_message_for_searching_codebase())
        }
...
fn render_search_action_input(
    ...
    let description_text = match action {
        ...
        AIAgentActionType::Grep {
            ...
            if queries.len() == 1 {
                tr_with(
                    "ai_block.grep_for_query_in_path",
                ...
                let patterns_list = queries
                    ...
                    .join("\n");
                tr_with(
                    "ai_block.grep_for_patterns_in_path",
        ...
        AIAgentActionType::FileGlobV2 {
            ...
            if patterns.len() == 1 {
                tr_with(
                    "ai_block.search_files_matching_pattern",
                ...
                let patterns_list = patterns
                    ...
                    .join("\n");
                tr_with(
                    "ai_block.find_files_matching_patterns",
                    &[("path", &display_path), ("patterns", &patterns_list)],
                )
            }
        }
