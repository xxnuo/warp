...
impl NotificationMailboxView {
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let mark_all_read_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(
                tr("ai.agent_management.notifications.mark_all_as_read"),
                NakedTheme,
            )
...
impl NotificationMailboxView {
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let label = appearance
            .ui_builder()
            .wrappable_text(tr("ai.agent_management.notifications.title"), false)
    ...
    fn render_empty_state(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        Container::new(
            appearance
                .ui_builder()
                .wrappable_text(
                    tr("ai.agent_management.notifications.no_notifications"),
                    false,
                )
                .with_style(UiComponentStyles {
                    font_size: Some(14.),
