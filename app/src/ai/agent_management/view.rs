...
fn agent_source_display_name(source: &AgentSource) -> String {
    match source {
        AgentSource::Linear => tr("ai.agent_management.source.linear"),
        AgentSource::AgentWebhook => tr("ai.agent_management.source.api"),
        AgentSource::Slack => tr("ai.agent_management.source.slack"),
        AgentSource::Cli => tr("ai.agent_management.source.cli"),
        AgentSource::ScheduledAgent => tr("ai.agent_management.source.scheduled"),
        AgentSource::Interactive | AgentSource::CloudMode => {
            tr("ai.agent_management.source.warp_app")
        }
        AgentSource::WebApp => tr("ai.agent_management.source.oz_web"),
        AgentSource::GitHubAction => tr("ai.agent_management.source.github_action"),
    }
}
...
impl AgentManagementView {
    pub fn new(
        ...
        let all_filter_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(tr("common.all"), NakedTheme)
                .with_size(ButtonSize::Small)
                .with_tooltip(tr("ai.agent_management.owner_all_tooltip"))
                .on_click(|ctx| {
        ...
        let personal_filter_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(tr("ai.agent_management.personal"), NakedTheme)
                .with_size(ButtonSize::Small)
                .with_tooltip(tr("ai.agent_management.personal_tooltip"))
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(AgentManagementViewAction::SetOwnerFilter(
        ...
        let setup_guide_button = CompactibleActionButton::new(
            tr("ai.agent_management.get_started"),
            None,
            ButtonSize::Small,
        ...
        let view_agents_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(tr("ai.agent_management.view_agents"), NakedTheme)
                .with_size(ButtonSize::Small)
        ...
        let no_filter_results_button = ctx.add_typed_action_view(move |_ctx| {
            ActionButton::new(tr("ai.agent_management.clear_filters"), SecondaryTheme)
                .with_size(ButtonSize::Small)
                .on_click(move |ctx| {
        ...
        let clear_all_filters_button = ctx.add_typed_action_view(move |_ctx| {
            ActionButton::new(tr("ai.agent_management.clear_all"), NakedTheme)
                .with_icon(Icon::X)
                .with_size(ButtonSize::Small)
                .on_click(move |ctx| {
                    ctx.dispatch_typed_action(AgentManagementViewAction::ClearFilters)
        ...
        let new_agent_button = CompactibleActionButton::new(
            tr("ai.agent_management.new_agent"),
            None,
            ButtonSize::Small,
    ...
    fn create_status_dropdown(
        ...
        Self::setup_filter_menu(&mut dropdown, tr("ai.agent_management.filter.status"), ctx);
        ...
        let items = vec![
            ...
            make_status_option(
                tr("ai.agent_management.status.working"),
    ...
    fn create_source_dropdown(
        ...
        let mut dropdown = Dropdown::new(ctx);
        Self::setup_filter_menu(&mut dropdown, tr("ai.agent_management.filter.source"), ctx);
    ...
    fn build_source_dropdown_items() -> Vec<MenuItem<DropdownAction>> {
        ...
        let mut items = vec![MenuItem::Item(
            MenuItemFields::new(tr("common.all")).with_on_select_action(
        ...
        for source in sources {
            items.push(MenuItem::Item(
                MenuItemFields::new(agent_source_display_name(&source)).with_on_select_action(
                    DropdownAction::select_action_and_close(
    ...
    fn create_created_on_dropdown(
        ...
        Self::setup_filter_menu(
            &mut dropdown,
            tr("ai.agent_management.filter.created_on"),
            ctx,
        );

        let items = vec![
            MenuItem::Item(MenuItemFields::new(tr("common.all")).with_on_select_action(
                DropdownAction::select_action_and_close(
                    AgentManagementViewAction::SetCreatedOnFilter(CreatedOnFilter::All),
                ),
            )),
            MenuItem::Item(
                MenuItemFields::new(tr("ai.agent_management.created_on.last_24_hours"))
            ...
            MenuItem::Item(
                MenuItemFields::new(tr("ai.agent_management.created_on.past_3_days"))
                    .with_on_select_action(DropdownAction::select_action_and_close(
                        AgentManagementViewAction::SetCreatedOnFilter(CreatedOnFilter::Past3Days),
                    )),
            ),
            MenuItem::Item(
                MenuItemFields::new(tr("ai.agent_management.created_on.last_week"))
                    .with_on_select_action(DropdownAction::select_action_and_close(
    ...
    fn create_artifact_dropdown(
        ...
        Self::setup_filter_menu(
            &mut dropdown,
            tr("ai.agent_management.filter.has_artifact"),
            ctx,
        );
        ...
        let items = vec![
            ...
            MenuItem::Item(
                MenuItemFields::new(tr("ai.agent_management.artifact.pull_request"))
                    .with_on_select_action(DropdownAction::select_action_and_close(
                        AgentManagementViewAction::SetArtifactFilter(ArtifactFilter::PullRequest),
                    )),
            ),
            MenuItem::Item(
                MenuItemFields::new(tr("ai.agent_management.artifact.plan")).with_on_select_action(
                    DropdownAction::select_action_and_close(
            ...
            MenuItem::Item(
                MenuItemFields::new(tr("ai.agent_management.artifact.screenshot"))
                    .with_on_select_action(DropdownAction::select_action_and_close(
                        AgentManagementViewAction::SetArtifactFilter(ArtifactFilter::Screenshot),
                    )),
            ),
            MenuItem::Item(
                MenuItemFields::new(tr("ai.agent_management.artifact.file")).with_on_select_action(
                    DropdownAction::select_action_and_close(
    ...
    fn create_harness_dropdown(
        ...
        let mut dropdown = Dropdown::new(ctx);
        Self::setup_filter_menu(&mut dropdown, tr("ai.agent_management.filter.harness"), ctx);
    ...
    fn create_environment_dropdown(
        ...
        let label = tr("ai.agent_management.filter.environment");
        Self::setup_searchable_filter_menu(&mut dropdown, label.clone(), ctx);
        ...
        let all = tr("common.all");
        let none = tr("ai.agent_management.none");
        dropdown.set_menu_header_text_override(move |text| {
            if text == all || text == none {
                return tr_with(
                    "ai.agent_management.filter_label",
            ...
            if abbreviated == text {
                ...
                tr_with(
                    "ai.agent_management.filter_label",
                    &[("label", &label), ("value", &format!("{abbreviated}…"))],
                )
            }
        });
    ...
    fn create_creator_dropdown(
        ...
        Self::setup_searchable_filter_menu(
            &mut dropdown,
            tr("ai.agent_management.filter.created_by"),
            ctx,
        );
        dropdown
    }
    ...
    fn update_environment_dropdown(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let selected_name = match &self.filters.environment {
            EnvironmentFilter::All => Some(tr("common.all")),
            EnvironmentFilter::NoEnvironment => Some(tr("ai.agent_management.none")),
        ...
        self.environment_dropdown.update(ctx, |dropdown, ctx| {
            ...
            items.push(MenuItem::Item(
                MenuItemFields::new(tr("ai.agent_management.none")).with_on_select_action(
                    DropdownAction::select_action_and_close(
    ...
    fn update_creator_dropdown(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let creator_filter_name = match &self.filters.creator {
            CreatorFilter::All => tr("common.all"),
        ...
        match event {
            ...
            ArtifactButtonsRowEvent::CopyBranch { branch } => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::default(tr("ai.artifacts.copy_branch_name"));
    ...
    fn render_session_status_label(
        ...
        let (label_text, tooltip_text_opt) = match session_status {
            SessionStatus::Expired => (
                tr("ai.agent_management.session_expired"),
                Some(tr("ai.agent_management.session_expired_tooltip")),
            ),
            SessionStatus::Unavailable => (tr("ai.agent_management.no_session_available"), None),
        ...
        Hoverable::new(mouse_state, move |state| {
            ...
            if state.is_hovered() {
                if let Some(tooltip_text) = tooltip_text_opt.clone() {
    ...
    fn render_metadata_row(
        ...
        if let Some(source) = &entry.display.source {
            let value = agent_source_display_name(source);
            metadata_parts.push(tr_with(
                "ai.agent_management.metadata_label",
                &[
                    ("label", &tr("ai.agent_management.filter.source")),
                    ("value", &value),
                ],
            ));
        }
        ...
        if availability.should_show_harness_selector() {
            if let Some(harness) = entry.display.harness {
                let value = availability.display_name_for(harness);
                metadata_parts.push(tr_with(
                    "ai.agent_management.metadata_label",
                    &[
                        ("label", &tr("ai.agent_management.filter.harness")),
                        ("value", &value),
                    ],
                ));
            }
        ...
        if let Some(executor) = &entry.display.executor {
            ...
            if !same_as_creator {
                if let Some(name) = executor.name.as_deref().or(executor.uid.as_deref()) {
                    ...
                    {
                        tr("ai.conversation_details.agent")
                    } else {
                        tr("ai.agent_management.executor")
                    };
                    metadata_parts.push(tr_with(
                        "ai.agent_management.metadata_label",
        ...
        if let Some(run_time) = &entry.display.run_time {
            metadata_parts.push(tr_with(
                "ai.agent_management.metadata_label",
                &[
                    ("label", &tr("ai.conversation_details.run_time")),
                    ("value", run_time),
                ],
            ));
        }
        ...
        if let Some(usage) = entry.display.request_usage.map(format_credits) {
            metadata_parts.push(tr_with(
                "ai.agent_management.metadata_label",
                &[
                    ("label", &tr("ai.conversation_details.credits_used")),
                    ("value", &usage),
                ],
            ));
        }
        ...
        Hoverable::new(self.loading_icon_mouse_state.clone(), move |mouse_state| {
            ...
            if mouse_state.is_hovered() {
                let tooltip = ui_builder
                    .tool_tip(tr("ai.agent_management.loading_cloud_agent_runs"))
                    .build()
                    .finish();
                stack.add_positioned_overlay_child(
                    tooltip,
    ...
    fn render_loading_header(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let loading_row = Flex::row()
            ...
            .with_child(
                Text::new_inline(
                    tr("ai.agent_management.loading_agents"),
                    appearance.ui_font_family(),
    ...
    fn render_no_results_view(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let text = Text::new_inline(
            tr("ai.agent_management.no_results_matched_filters"),
            appearance.ui_font_family(),
