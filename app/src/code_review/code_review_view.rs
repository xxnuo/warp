    ...
    F: Fn(&mut warpui::EventContext<'_>) + 'static,
{
    ...
    .with_tooltip(move || {
        let tooltip = if is_sidebar_expanded {
            tr("code_review.hide_file_navigation")
        } else {
            tr("code_review.show_file_navigation")
        };
...
pub fn code_review_tooltip_text() -> String {
    tr("code_review.view_changes")
}

pub fn get_discard_button_disabled_tooltip(git_operation_blocked: bool) -> String {
    if git_operation_blocked {
        tr("code_review.discard_disabled_git_operation")
    } else {
        tr("code_review.no_changes_to_discard")
    }
}
...
impl DiscardOperationType {
    pub fn title(&self) -> String {
        match self {
            DiscardOperationType::AllUncommittedChanges => {
                tr("code_review.discard_uncommitted_title")
            }
            DiscardOperationType::FileUncommittedChanges => {
                tr("code_review.discard_file_uncommitted_title")
            }
            DiscardOperationType::AllChangesAgainstBranch(_) => {
                tr("code_review.discard_all_changes_title")
            }
            DiscardOperationType::FileChangesAgainstBranch(_) => {
                tr("code_review.discard_file_changes_title")
            }
        }
    }
    ...
    pub fn description(&self) -> Option<String> {
        match self {
            DiscardOperationType::AllUncommittedChanges => {
                Some(tr("code_review.discard_uncommitted_description"))
            }
            DiscardOperationType::FileUncommittedChanges => {
                Some(tr("code_review.discard_file_uncommitted_description"))
            }
            DiscardOperationType::AllChangesAgainstBranch(None) => {
                Some(tr("code_review.discard_all_changes_description"))
            }
            DiscardOperationType::FileChangesAgainstBranch(None) => {
                Some(tr("code_review.discard_file_changes_main_description"))
            }
            DiscardOperationType::AllChangesAgainstBranch(Some(_)) => {
                Some(tr("code_review.discard_all_changes_description"))
            }
            DiscardOperationType::FileChangesAgainstBranch(Some(branch)) => Some(tr_with(
...
impl CodeReviewView {
    ...
    pub fn new(
        ...
        let maximize_button = ctx.add_typed_action_view(move |_| {
            ActionButton::new("", NakedTheme)
                ...
                .with_tooltip(tr("code_review.maximize"))
        ...
        let git_primary_action_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(tr("code_review.commit"), SecondaryTheme)
        ...
        let undo_action_button = ctx.add_typed_action_view(move |ctx| {
            ...
            let mut action_button = ActionButton::new(tr("common.undo"), NakedTheme)
                .with_size(ButtonSize::Small)
                .on_click(move |ctx| {
        ...
        let discard_confirm_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(tr("common.discard_changes"), DangerPrimaryTheme)
        ...
        let init_project_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(tr("code_review.initialize_codebase"), NakedTheme)
                .with_size(ButtonSize::Small)
                .with_tooltip(tr("code_review.initialize_codebase_tooltip"))
        ...
        let open_repository_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(tr("code_review.open_repository"), NakedTheme)
                .with_size(ButtonSize::Small)
                .with_tooltip(tr("code_review.open_repository_tooltip"))
                .with_tooltip_alignment(TooltipAlignment::Center)
        ...
        for file in files {
            ...
            let open_in_tab_button = ctx.add_typed_action_view(move |_ctx| {
                ActionButton::new("", NakedTheme)
                    ...
                    .with_size(ButtonSize::InlineActionHeader)
                    .with_tooltip(tr("common.open_file"))
                    .on_click(move |ctx| {
            ...
            let add_context_button = ctx.add_typed_action_view(move |_ctx| {
                ActionButton::new("", NakedTheme)
                    ...
                    .with_size(ButtonSize::InlineActionHeader)
                    .with_tooltip(tr("code_review.add_file_diff_as_context"))
                    .on_click(move |ctx| {
            ...
            let copy_path_button = ctx.add_typed_action_view(move |_ctx| {
                ActionButton::new("", NakedTheme)
                    .with_icon(Icon::Copy)
                    .with_size(ButtonSize::InlineActionHeader)
                    .with_tooltip(tr("code_review.copy_file_path"))
                    .on_click(move |ctx| {
    ...
    fn render_placeholder_header(appearance: &Appearance) -> Box<dyn Element> {
        ...
        let header_text = tr("code_review.loading_open_changes");
    ...
    fn render_error_state(&self, error: &str, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let main_column = Flex::column()
            ...
            .with_child(
                Text::new(
                    tr("code_review.error_loading_diffs"),
                    appearance.ui_font_family(),
    ...
    pub fn render_no_repo_found_state(
        ...
        let main_column = Flex::column()
            ...
            .with_child(
                Text::new(
                    tr("code_review.cannot_detect_diffs"),
                    appearance.ui_font_family(),
                    appearance.ui_font_size() + 2.,
                )
                .with_style(Properties::default().weight(Weight::Semibold))
    ...
    pub fn render_remote_state(
        ...
        Self::render_no_repo_found_state(
            appearance,
            tr("code_review.remote_workspaces_unsupported"),
            open_repo_button,
        )
    }
    ...
    pub fn render_wsl_state(
        ...
        Self::render_no_repo_found_state(
            appearance,
            tr("code_review.wsl_unsupported"),
            open_repo_button,
        )
    }
    ...
    pub fn render_not_repo_state(
        ...
        Self::render_no_repo_found_state(
            appearance,
            tr("code_review.git_repositories_only"),
            open_repo_button,
        )
    }
    ...
    fn render_no_changes_state(
        ...
        let mut zero_state_column = Flex::column()
            ...
            .with_child(
                Text::new(
                    tr("code_review.no_open_changes"),
                    appearance.ui_font_family(),
                    16.,
                )
                .with_style(Properties::default().weight(Weight::Semibold))
            ...
            .with_child(
                Container::new(
                    Text::new(
                        tr("code_review.no_open_changes_description"),
                        appearance.ui_font_family(),
                        14.,
                    )
                    ...
                    if let Some(file_name) = first_rule.path.file_name() {
                        zero_state_column.add_child(
                            Container::new(
                                Text::new(
                                    tr_with(
                                        "code_review.repo_initialized_with_file",
                                        &[("file", file_name)],
                                    ),
                                    appearance.ui_font_family(),
                                    12.,
        ...
        match result {
            ReviewSubmissionResult::Success {
                ...
                ToastStack::handle(ctx).update(ctx, |stack, ctx| {
                    let toast = DismissibleToast::default(tr("code_review.comments_sent_to_agent"));
            ...
            ReviewSubmissionResult::Error => {
                ...
                let error_message = tr("code_review.could_not_submit_comments");
        ...
        left_section.add_child(if let Some(editor_state) = &file.editor_state {
            if editor_state.has_unsaved_changes(app) {
                ...
                let tooltip_text = tr_with(
                    "code_review.unsaved_file_tooltip",
    ...
    fn render_file_content(&self, file: &FileState, appearance: &Appearance) -> Box<dyn Element> {
        ...
        if file.file_diff.is_binary {
            Self::styled_file_content_container(
                Text::new(
                    tr("code_review.binary_file_no_diff"),
                    appearance.monospace_font_family(),
                    appearance.monospace_font_size(),
                )
                .with_color(theme.main_text_color(theme.background()).into())
            ...
            Self::styled_file_content_container(
                Text::new(
                    tr("code_review.file_renamed_without_changes"),
                    appearance.monospace_font_family(),
                    appearance.monospace_font_size(),
                )
                .with_color(theme.main_text_color(theme.background()).into())
            ...
            Self::styled_file_content_container(
                Text::new(
                    tr("code_review.unable_to_load_file_content"),
                    appearance.ui_font_family(),
                    appearance.ui_font_size(),
                )
                .with_color(theme.sub_text_color(theme.background()).into())
    ...
    fn render_file_row(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if self.discard_dialog_state.discard_file_paths.is_empty() {
            return Text::new(
                tr("code_review.no_file_selected"),
                appearance.ui_font_family(),
                appearance.ui_font_size(),
            )
            .with_color(
        ...
        let CodeReviewViewState::Loaded(loaded) = self.state() else {
            return Text::new(
                tr("code_review.no_files_to_discard"),
                appearance.ui_font_family(),
                appearance.ui_font_size(),
            )
            .with_color(
            ...
            let stash_checkbox = Container::new(
                appearance
                    ...
                    .with_label(
                        appearance
                            .ui_builder()
                            .span(tr("code_review.stash_changes"))
                            .with_style(UiComponentStyles {
        ...
        match event {
            ...
            CodeEditorEvent::DiffReverted => {
                ...
                crate::workspace::ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = crate::view_components::DismissibleToast::default(tr(
            ...
            if !is_input_box_visible {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::default(tr(
                        "code_review.cannot_attach_diff_input_unavailable",
                    ))
                    .with_object_id(toast_id);
                    toast_stack.add_ephemeral_toast(toast, self.window_id, ctx);
                });
    ...
    fn update_git_operations_ui(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if self.repo_path().is_some_and(LocalOrRemotePath::is_remote) {
            let remote_tooltip = tr("code_review.git_operations_remote_unavailable");
            self.git_primary_action_button.update(ctx, |button, ctx| {
                button.set_label(tr("code_review.commit"), ctx);
        ...
        match mode {
            PrimaryGitActionMode::Commit => {
                ...
                self.git_primary_action_button.update(ctx, |button, ctx| {
                    ...
                    button.set_tooltip(
                        disabled.then_some(tr("code_review.no_changes_to_commit")),
                        ctx,
                    );
                    button.set_on_click(
                ...
                self.git_operations_chevron.update(ctx, |button, ctx| {
                    button.set_disabled(disabled, ctx);
                    button.set_tooltip(
                        disabled.then_some(tr("code_review.no_git_actions_available")),
                        ctx,
                    );
                });
            }
            PrimaryGitActionMode::Push => {
                self.git_primary_action_button.update(ctx, |button, ctx| {
                    button.set_label(tr("code_review.push"), ctx);
            ...
            PrimaryGitActionMode::CreatePr => {
                self.git_primary_action_button.update(ctx, |button, ctx| {
                    button.set_label(tr("code_review.create_pr"), ctx);
            ...
            PrimaryGitActionMode::ViewPr => {
                ...
                if let Some(pr_info) = pr_info {
                    ...
                    self.git_primary_action_button.update(ctx, |button, ctx| {
                        ...
                        button.set_tooltip(
                            is_pr_info_refreshing.then_some(tr("code_review.refreshing_pr_info")),
                            ctx,
                        );
                        button.set_on_click(
                            move |ctx| {
            ...
            PrimaryGitActionMode::Publish => {
                self.git_primary_action_button.update(ctx, |button, ctx| {
                    button.set_label(tr("code_review.publish"), ctx);
    ...
    fn commit_menu_item(disabled: bool) -> MenuItem<CodeReviewAction> {
        MenuItemFields::new(tr("code_review.commit"))
            .with_icon(Icon::GitCommit)
    ...
    fn push_or_publish_menu_item(has_upstream: bool, disabled: bool) -> MenuItem<CodeReviewAction> {
        if has_upstream {
            MenuItemFields::new(tr("code_review.push"))
                .with_icon(Icon::ArrowUp)
                .with_on_select_action(CodeReviewAction::OpenPushDialog)
                .with_disabled(disabled)
                .into_item()
        } else {
            MenuItemFields::new(tr("code_review.publish"))
    ...
    fn pr_menu_item(&self, app: &AppContext) -> MenuItem<CodeReviewAction> {
        ...
        if let Some(pr_info) = self.pr_info(app) {
            ...
            let has_upstream = diff_state.upstream_ref(app).is_some();
            let upstream_differs_from_main = diff_state.upstream_differs_from_main(app);
            MenuItemFields::new(tr("code_review.create_pr"))
                .with_icon(Icon::Github)
    ...
    fn header_menu_items_legacy(
        ...
        if FeatureFlag::DiffSetAsContext.is_enabled() && has_changes {
            items.push(
                MenuItemFields::new(tr("code_review.add_diff_set_as_context"))
                    .with_icon(Icon::Paperclip)
        ...
        let (comment_label, comment_icon) = if self.get_existing_diffset_comment(ctx).is_some() {
            (tr("code_review.show_saved_comment"), Icon::MessageText)
        } else {
            (tr("code_review.add_comment"), Icon::MessagePlusSquare)
        };
    ...
    fn header_menu_items_new(
        ...
        if FeatureFlag::DiscardPerFileAndAllChanges.is_enabled() && has_changes {
            items.push(
                MenuItemFields::new(tr("code_review.discard_all"))
                    .with_icon(Icon::ReverseLeft)
