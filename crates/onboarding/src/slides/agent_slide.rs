...
impl AgentSlide {
    ...
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        let title = appearance
            .ui_builder()
            .paragraph(tr("onboarding.agent.title"))
    ...
    fn render_model_section(
        ...
        let header = self.render_section_header(tr("onboarding.agent.default_model"), appearance);
    ...
    fn render_model_row(
        ...
        let hoverable_body = Hoverable::new(mouse_state, move |_| {
            ...
            let trailing: Box<dyn Element> = if is_default {
                make_pill(tr("common.recommended"))
            } else if requires_upgrade {
                make_pill(tr("common.premium"))
            } else {
                Empty::new().finish()
            };
    ...
    fn render_autonomy_workspace_enforced(&self, appearance: &Appearance) -> Box<dyn Element> {
        let header = self.render_section_header(tr("onboarding.agent.autonomy"), appearance);
        ...
        let title_el = Text::new(
            tr("onboarding.agent.autonomy.team_workspace.title"),
            ui_font_family,
            14.0,
        )
        .with_color(title_color)
        ...
        let subtitle_el = Text::new(
            tr("onboarding.agent.autonomy.team_workspace.description"),
            ui_font_family,
            12.0,
        )
    ...
    fn render_autonomy_section(
        ...
        let autonomy_options: [(AgentAutonomy, String, String, MouseStateHandle); 3] = [
            (
                AgentAutonomy::Full,
                tr("onboarding.agent.autonomy.full.title"),
                tr("onboarding.agent.autonomy.full.description"),
                self.autonomy_full_mouse_state.clone(),
            ),
            (
                AgentAutonomy::Partial,
                tr("onboarding.agent.autonomy.partial.title"),
                tr("onboarding.agent.autonomy.partial.description"),
                self.autonomy_partial_mouse_state.clone(),
            ),
            (
                AgentAutonomy::None,
                tr("onboarding.agent.autonomy.none.title"),
                tr("onboarding.agent.autonomy.none.description"),
    ...
    fn render_disable_oz_section(
        ...
        let label = Text::new(
            tr("onboarding.agent.disable_warp_agent"),
            appearance.ui_font_family(),
            14.0,
        )
        .with_color(internal_colors::text_sub(theme, background_for_text))
    ...
    fn render_bottom_nav(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let next_button = self.next_button.render(
            appearance,
            button::Params {
                content: button::Content::Label(tr("common.next").into()),
    ...
    fn render_upgrade_banner(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let title = Text::new(
            tr("onboarding.agent.upgrade_banner.title"),
            ui_font_family,
            13.0,
        )
        ...
        let subtitle = Text::new(
            tr("onboarding.agent.upgrade_banner.subtitle"),
            ui_font_family,
            12.0,
        )
        .with_color(internal_colors::text_sub(theme, background_for_text))
        ...
        let upgrade_button = self.upgrade_button.render(
            appearance,
            button::Params {
                content: button::Content::Label(tr("common.upgrade").into()),
    ...
    fn render_auth_prompt_bar(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let copy_url_link = ui_builder
            .link(
                tr("onboarding.agent.auth_prompt.copy_url"),
                None,
                Some(Box::new(|ctx| {
        ...
        let paste_token_link = ui_builder
            .link(
                tr("onboarding.agent.auth_prompt.click_here"),
                None,
                Some(Box::new(|ctx| {
        ...
        let text_row = Flex::row()
            ...
            .with_child(
                Container::new(
                    ui_builder
                        .span(tr("onboarding.agent.auth_prompt.prefix"))
            ...
            .with_child(
                ui_builder
                    .span(tr("onboarding.agent.auth_prompt.middle"))
                    .with_style(text_styles)
                    .build()
                    .finish(),
            )
            ...
            .with_child(
                ui_builder
                    .span(tr("onboarding.agent.auth_prompt.suffix"))
                    .with_style(text_styles)
                    .build()
                    .finish(),
            )
    ...
    fn render_plan_activated_toast(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let text = ui_builder
            .span(tr("onboarding.agent.plan_activated"))
            .with_style(UiComponentStyles {
                font_color: Some(text_color),
                font_size: Some(FONT_SIZE),
                font_weight: Some(Weight::Medium),
